...
export class AnalyticsEngine {
  ...
  private countAtRiskClubs(entry: DistrictCacheEntry): number {
    return entry.clubPerformance.filter(club => {
      const membership = parseInt(club['Active Membership'] || club['Membership'] || '0')
  ...
  private async analyzeClubTrends(
    ...
    for (const club of latestEntry.clubPerformance) {
      ...
      clubMap.set(clubId, {
        clubId,
        clubName,
        divisionId: club['Division'] || '',
        divisionName: club['Division Name'] || '',
        areaId: club['Area'] || '',
        areaName: club['Area Name'] || '',
        membershipTrend: [],
        dcpGoalsTrend: [],
  ...
  private analyzeDivisions(dataEntries: DistrictCacheEntry[]): DivisionAnalytics[] {
    ...
    for (const club of latestEntry.clubPerformance) {
      const divisionId = club['Division'] || ''
      const divisionName = club['Division Name'] || divisionId
  ...
  private analyzeAreas(entry: DistrictCacheEntry): AreaAnalytics[] {
    ...
    for (const club of entry.clubPerformance) {
      const areaId = club['Area'] || ''
      const areaName = club['Area Name'] || areaId
      const divisionId = club['Division'] || ''
  ...
  private analyzeDCPGoals(entry: DistrictCacheEntry): {
    ...
    // Note: We're inferring goal achievement from total goals met
    // In reality, we'd need individual goal data from the CSV
    for (const club of entry.clubPerformance) {
      const goalsAchieved = parseInt(club['Goals Met'] || club['DCP Goals'] || '0')
      
      // Assume clubs achieve goals sequentially (1, 2, 3, etc.)
      // This is a simplification - actual data may vary
      for (let i = 0; i < Math.min(goalsAchieved, 10); i++) {
        goalCounts[i]++
      }
    }
    ...
    for (const [divisionId, history] of divisionPerformance.entries()) {
      ...
      for (let i = 2; i < history.length; i++) {
        ...
        if (Math.abs(delta) >= beforeAvg * 0.2 && beforeAvg > 0) {
          ...
          const divisionName = dataEntries[dataEntries.length - 1].clubPerformance
