  ...
  const generateInvalidConfig = (
    ...
    switch (violationType) {
      case 'maxReconciliationDays_negative':
        return {
          ...baseConfig,
          maxReconciliationDays: -Math.floor(seed * 10) - 1,
        }
      ...
      case 'maxReconciliationDays_too_large':
        return {
          ...baseConfig,
          maxReconciliationDays: Math.floor(seed * 40) + 61,
        } // 61-100
      ...
      case 'stabilityPeriodDays_negative':
        return {
          ...baseConfig,
          stabilityPeriodDays: -Math.floor(seed * 10) - 1,
        }
      ...
      case 'stabilityPeriodDays_exceeds_max':
        return {
          ...baseConfig,
          stabilityPeriodDays:
            baseConfig.maxReconciliationDays + Math.floor(seed * 10) + 1,
        }
      case 'checkFrequencyHours_negative':
        return {
          ...baseConfig,
          checkFrequencyHours: -Math.floor(seed * 10) - 1,
        }
      ...
      case 'checkFrequencyHours_too_large':
        return {
          ...baseConfig,
          checkFrequencyHours: Math.floor(seed * 100) + 169,
        } // > 168
  ...
  describe('Property 8: Configuration Compliance', () => {
    ...
    it('should reject configurations with invalid maxReconciliationDays (Requirement 6.1)', () => {
      ...
      for (const violationType of invalidTypes) {
        for (let i = 0; i < 25; i++) {
          ...
          expect(errors.some(e => e.field === 'maxReconciliationDays')).toBe(
            true
          )
        }
      }
        ...
        for (let i = 0; i < 15; i++) {
          ...
          if (violationType.includes('membershipPercent')) {
            ...
            expect(
              errors.some(
                e =>
                  e.field === 'significantChangeThresholds.distinguishedPercent'
              )
            ).toBe(true)
          }
        }
      ...
      for (const violationType of invalidTypes) {
        for (let i = 0; i < 25; i++) {
          ...
          if (violationType.includes('autoExtensionEnabled')) {
            expect(errors.some(e => e.field === 'autoExtensionEnabled')).toBe(
              true
            )
    ...
    it('should validate threshold structure completeness (Requirement 6.2)', () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        expect(
          typeof config.significantChangeThresholds.membershipPercent
        ).toBe('number')
        expect(
          typeof config.significantChangeThresholds.clubCountAbsolute
        ).toBe('number')
        expect(
          typeof config.significantChangeThresholds.distinguishedPercent
        ).toBe('number')
        ...
        expect(
          Number.isInteger(config.significantChangeThresholds.clubCountAbsolute)
        ).toBe(true)
    ...
    it('should handle missing significantChangeThresholds gracefully (Requirement 6.2)', () => {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        expect(errors.length).toBeGreaterThan(0)
        expect(
          errors.some(e => e.field === 'significantChangeThresholds')
        ).toBe(true)
      }
    })
