...
describe('ReconciliationOrchestrator - Property-Based Tests', () => {
  ...
  const generateConfig = (seed: number = Math.random()): ReconciliationConfig => ({
  ...
  const generateTimelineWithStablePeriod = (
    ...
    if (hasRecentSignificantChanges && stableDays > 0) {
      const significantChangeDate = new Date(now.getTime() - ((stableDays + 1) * 24 * 60 * 60 * 1000))
  ...
  describe('Property 10: Extension Logic', () => {
    it('should automatically extend reconciliation when significant changes detected near end', async () => {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        const membershipIncrease = Math.max(
          ...
          Math.floor(config.significantChangeThresholds.membershipPercent * cachedData.membership.total / 100) + 1
        )
        currentData.membership.total = cachedData.membership.total + membershipIncrease
        currentData.membership.change = membershipIncrease
        currentData.membership.changePercent = (membershipIncrease / cachedData.membership.total) * 100
        currentData.asOfDate = now.toISOString().split('T')[0]

        // Process reconciliation cycle (this should trigger extension)
        const status = await orchestrator.processReconciliationCycle(job.id, currentData, cachedData)
        ...
        console.log(`Test ${i}: originalMaxEndTime=${originalMaxEndTime}, updatedMaxEndTime=${updatedJob!.maxEndDate.getTime()}`)
        
        // The max end date should have been extended beyond the original time
        expect(updatedJob!.maxEndDate.getTime()).toBeGreaterThan(originalMaxEndTime)
        
        // The extension should not exceed the configured maximum
        const extensionMs = updatedJob!.maxEndDate.getTime() - originalMaxEndTime
    ...
    it('should not extend reconciliation when auto-extension is disabled', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const cachedData = generateDistrictStatistics(seed)
        const currentData = generateDistrictStatistics(seed + 0.1)
        currentData.membership.total = cachedData.membership.total + Math.floor(cachedData.membership.total * 0.1) // 10% increase
        currentData.asOfDate = now.toISOString().split('T')[0]

        // Process reconciliation cycle
        const originalMaxEndDate = new Date(job.maxEndDate)
        await orchestrator.processReconciliationCycle(job.id, currentData, cachedData)

        // Property: When auto-extension is disabled, reconciliation should not be extended
        // even with significant changes near the end
        const updatedJob = await storageManager.getJob(job.id)
        expect(updatedJob).toBeDefined()
        expect(updatedJob!.maxEndDate.getTime()).toBe(originalMaxEndDate.getTime())
      }
    })
    ...
    it('should support manual extension with proper limit enforcement', async () => {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        const extensionDays = Math.min(config.maxExtensionDays, Math.floor(seed * 5) + 1) // Within limits
        ...
        const expectedEndDate = new Date(originalMaxEndDate.getTime() + (extensionDays * 24 * 60 * 60 * 1000))
        expect(extendedJob!.maxEndDate.getTime()).toBe(expectedEndDate.getTime())

        // Test that we can extend multiple times up to the total limit
        if (extensionDays < config.maxExtensionDays) {
          const additionalExtension = Math.min(1, config.maxExtensionDays - extensionDays)
          if (additionalExtension > 0) {
            ...
            const expectedSecondEndDate = new Date(beforeSecondExtension.getTime() + (additionalExtension * 24 * 60 * 60 * 1000))
            expect(doubleExtendedJob!.maxEndDate.getTime()).toBe(expectedSecondEndDate.getTime())
          }
        }
      }
    })
    ...
    it('should not extend reconciliation when not close to max end date', async () => {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const cachedData = generateDistrictStatistics(seed)
        const currentData = generateDistrictStatistics(seed + 0.1)
        currentData.membership.total = cachedData.membership.total + Math.floor(cachedData.membership.total * 0.08) // 8% increase
        currentData.asOfDate = now.toISOString().split('T')[0]
    ...
    it('should handle extension attempts on non-active jobs gracefully', async () => {
      ...
      const testStatuses: Array<'completed' | 'failed' | 'cancelled'> = ['completed', 'failed', 'cancelled']
      ...
      for (let i = 0; i < testStatuses.length; i++) {
        ...
        await expect(orchestrator.extendReconciliation(job.id, 2)).resolves.not.toThrow()
        ...
        expect(unchangedJob!.maxEndDate.getTime()).toBe(originalMaxEndDate.getTime())
      }
    })
    ...
    it('should maintain extension invariants across different scenarios', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        expect(extendedJob!.startDate.getTime()).toBe(originalStartDate.getTime())
        ...
        const timeSinceUpdate = Date.now() - extendedJob!.metadata.updatedAt.getTime()
        expect(timeSinceUpdate).toBeLessThan(5000) // Within 5 seconds
        
        // Invariant 6: Max end date should always be after start date
        expect(extendedJob!.maxEndDate.getTime()).toBeGreaterThan(extendedJob!.startDate.getTime())
      }
    })
    ...
    it('should handle edge cases in extension logic correctly', async () => {
      ...
      const expectedEnd1 = new Date(originalEnd1.getTime() + (24 * 60 * 60 * 1000))
      ...
      await expect(orchestrator.extendReconciliation('nonexistent-job', 1))
  ...
  describe('Property 4: Finalization Logic', () => {
    it('should finalize reconciliation when stability period is met', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        await expect(orchestrator.finalizeReconciliation(job.id)).resolves.not.toThrow()
        ...
        expect(finalizedTimeline!.status.daysStable).toBe(config.stabilityPeriodDays)
      }
    })
    ...
    it('should maintain finalization invariants across different timeline patterns', async () => {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        if (i % 3 === 0) {
          ...
          timeline = generateTimelineWithStablePeriod(job.id, districtId, targetMonth, 2)
        } else if (i % 3 === 1) {
          // Pattern 2: More than required stability period
          timeline = generateTimelineWithStablePeriod(job.id, districtId, targetMonth, 4)
        } else {
          // Pattern 3: Required stability period with older significant changes
          timeline = generateTimelineWithStablePeriod(job.id, districtId, targetMonth, 2, true)
        }
        ...
        expect(finalizedTimeline!.status.daysStable).toBeGreaterThanOrEqual(config.stabilityPeriodDays)
        
        // Invariant 5: Finalized date should be after or equal to start date
        expect(finalizedJob!.finalizedDate!.getTime()).toBeGreaterThanOrEqual(job.startDate.getTime())
        
        // Invariant 6: End date should be after or equal to start date
        expect(finalizedJob!.endDate!.getTime()).toBeGreaterThanOrEqual(job.startDate.getTime())
      }
    })
    ...
    it('should respect maximum reconciliation period in finalization logic', async () => {
      ...
      const timeline = generateTimelineWithStablePeriod(job.id, districtId, targetMonth, 1) // Less than required 3 days
      await storageManager.saveTimeline(timeline)
