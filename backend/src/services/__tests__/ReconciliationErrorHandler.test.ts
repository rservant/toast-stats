...
describe('ReconciliationErrorHandler', () => {
  ...
  describe('executeDashboardFetch', () => {
    it('should execute successful dashboard fetch', async () => {
      ...
      const mockCircuitBreaker = mockCircuitManager.getCircuitBreaker('reconciliation-dashboard')
      vi.mocked(mockCircuitBreaker.execute).mockImplementation(async (operation) => {
      ...
      expect(mockBackfillService.fetchReconciliationData).toHaveBeenCalledWith('D123', '2024-01-31')
    })
  ...
  describe('executeCacheOperation', () => {
    it('should execute successful cache operation', async () => {
      ...
      // Mock circuit breaker to execute operation directly
      const mockCircuitManager = vi.mocked(CircuitBreakerManager.getInstance)()
      const mockCircuitBreaker = mockCircuitManager.getCircuitBreaker('reconciliation-cache')
      vi.mocked(mockCircuitBreaker.execute).mockImplementation(async (operation) => {
        return await operation()
      })
      
      const result = await errorHandler.executeCacheOperation(
        mockOperation,
        { districtId: 'D123', operation: 'cache-test' }
      )
    ...
    it('should handle cache operation failure', async () => {
      const mockOperation = vi.fn().mockRejectedValue(new Error('Cache write failed'))
      
      // Mock circuit breaker to throw error
      const mockCircuitManager = vi.mocked(CircuitBreakerManager.getInstance)()
      const mockCircuitBreaker = mockCircuitManager.getCircuitBreaker('reconciliation-cache')
      vi.mocked(mockCircuitBreaker.execute).mockRejectedValue(new Error('Cache write failed'))
  ...
  describe('error tracking and alerting', () => {
    ...
    it('should handle reconciliation timeout', async () => {
      ...
      expect(mockAlertManager.sendReconciliationTimeoutAlert).toHaveBeenCalledWith(
        'D123',
        '2024-01',
        20,
        15
    ...
    it('should track consecutive failures and send alerts', async () => {
      ...
      const mockCircuitManager = vi.mocked(CircuitBreakerManager.getInstance)()
      const mockCircuitBreaker = mockCircuitManager.getCircuitBreaker('reconciliation-cache')
      vi.mocked(mockCircuitBreaker.execute).mockRejectedValue(new Error('Persistent failure'))
      
      // Execute operation multiple times to trigger consecutive failure alert
      for (let i = 0; i < 4; i++) {
        await errorHandler.executeCacheOperation(
          mockOperation,
          { districtId: 'D123', operation: 'failing-operation' }
        )
      }
  ...
  describe('alerting configuration', () => {
    it('should not send alerts when alerting is disabled', async () => {
      ...
      expect(mockAlertManager.sendReconciliationFailureAlert).not.toHaveBeenCalled()
    })
  })
})