  ...
  describe('Property 7: Reconciliation Timeline Accuracy', () => {
    it('should accurately record and retrieve timeline entries', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.array(generateDataChanges(), { minLength: 1, maxLength: 3 }), // Further reduced for performance
          async (job, changesArray) => {
            ...
            const freshStorageManager = new ReconciliationStorageManager(
              await createTestCacheDir(testId)
            )
            ...
            try {
              ...
              // Ensure job is properly saved before proceeding
              await freshStorageManager.flush()
        ...
        ),
        {
          ...
          timeout: 2000, // 2 second timeout per property test
        }
      )
    ...
    it('should correctly calculate stability periods from timeline entries', async () => {
      await fc.assert(
        fc.asyncProperty(
          generateReconciliationJob(),
          fc.integer({ min: 0, max: 1 }), // Reduced to 1 for faster execution
          fc.integer({ min: 0, max: 1 }), // Reduced to 1 for faster execution
          async (job, stableDays, significantChangesBefore) => {
            ...
            const savedJob = await freshStorageManager.getJob(job.id)
            if (!savedJob) {
              throw new Error(`Job ${job.id} was not saved properly`)
            }
    ...
    it(
      'should provide accurate completion estimates based on timeline patterns',
      { timeout: 10000 },
      async () => {
        await fc.assert(
          fc.asyncProperty(
    ...
    it(
      'should handle timeline updates idempotently',
      ...
      async () => {
        await fc.assert(
          fc.asyncProperty(
            ...
            async (job, changes) => {
              ...
              try {
                ...
                const savedJob = await freshStorageManager.getJob(job.id)
                if (!savedJob) {
                  throw new Error(`Failed to save job: ${job.id}`)
                }
          ...
          { numRuns: 3 } // Reduced from 5 to 3
        )
      }
    )

    it(
      'should correctly identify significant changes based on thresholds',
      { timeout: 10000 },
      async () => {
        await fc.assert(
          fc.asyncProperty(
            ...
            async (
              ...
              try {
                ...
                const actualMembershipPercentChange =
                  ((membershipCurrent - membershipPrevious) /
                    membershipPrevious) *
                  100
    ...
    it(
      'should maintain timeline integrity across multiple operations',
      { timeout: 15000 },
      async () => {
        await fc.assert(
          fc.asyncProperty(
            generateReconciliationJob(),
            fc.array(generateDataChanges(), { minLength: 2, maxLength: 3 }), // Further reduced array size for performance
            async (job, changesArray) => {
              // Ensure job has valid date relationships
              if (job.endDate && job.endDate < job.startDate) {
                job.endDate = new Date(
              ...
              try {
                ...
                expect(stats.significantChanges).toBe(
    ...
    it(
      'should accurately detect stability periods and provide detailed stability information',
      { timeout: 10000 },
      async () => {
        await fc.assert(
          fc.asyncProperty(
            generateReconciliationJob(),
            fc.integer({ min: 0, max: 2 }), // significantChangesBefore - reduced
            fc.integer({ min: 1, max: 3 }), // stableDays - reduced
            fc.integer({ min: 0, max: 1 }), // additionalSignificantChangesAfter - reduced
            async (
              job,
              ...
              try {
                ...
                // Validate and fix job dates to ensure they make sense
                const now = new Date()
                ...
                // Ensure endDate is after startDate if it exists
                if (job.endDate && job.endDate < job.startDate) {
                  job.endDate = new Date(
                    job.startDate.getTime() + 24 * 60 * 60 * 1000
                  ) // 1 day after start
                ...
                if (job.finalizedDate) {
                  ...
                  // For active jobs, ensure status is active
                  job.status = 'active'
                }
                ...
                for (let i = 0; i < additionalSignificantChangesAfter; i++) {
                  const significantChanges: DataChanges = {
                    ...
                    clubCountChange: {
                      ...
                      absoluteChange:
                        job.config.significantChangeThresholds
                          .clubCountAbsolute + 1, // Ensure it exceeds threshold
                    },
                    timestamp: currentDate,
                    sourceDataDate: currentDate.toISOString().split('T')[0],
                  }
                ...
                const timeline =
                  await freshProgressTracker.getReconciliationTimeline(job.id)
                const stabilityInfo =
                  freshProgressTracker.getStabilityPeriodInfo(
                    timeline,
                    job.config.stabilityPeriodDays
                  )
                ...
                expect(
                  stats.stabilityPeriod.stabilityPeriodProgress
                ).toBeCloseTo(stabilityInfo.stabilityPeriodProgress, 2)
