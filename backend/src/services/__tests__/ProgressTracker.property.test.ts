...
describe('ProgressTracker - Property-Based Tests', () => {
  ...
  const generateDataChanges = (): fc.Arbitrary<DataChanges> =>
    fc.record({
      ...
      timestamp: fc.integer({ min: Date.parse('2024-01-01T00:00:00.000Z'), max: Date.parse('2024-12-30T23:59:59.999Z') })
  ...
  const generateReconciliationJob = (): fc.Arbitrary<ReconciliationJob> =>
    fc.record({
      id: fc.string({ minLength: 10, maxLength: 20 }).map(s => `job-${s.replace(/[^a-zA-Z0-9]/g, 'x')}`),
      districtId: fc.string({ minLength: 2, maxLength: 5 }).map(s => `D${s.replace(/[^a-zA-Z0-9]/g, 'x')}`),
      ...
      metadata: fc.record({
        createdAt: fc.integer({ min: Date.parse('2024-01-01T00:00:00.000Z'), max: Date.parse('2024-06-30T23:59:59.999Z') })
          .map(ms => createValidDate(ms)),
        updatedAt: fc.integer({ min: Date.parse('2024-01-01T00:00:00.000Z'), max: Date.parse('2024-12-31T23:59:59.999Z') })
          .map(ms => createValidDate(ms)),
      ...
      if (job.maxEndDate.getTime() < startTime) {
        job.maxEndDate = createValidDate(startTime, job.config.maxReconciliationDays)
      }
      ...
      if (job.metadata.updatedAt.getTime() < job.metadata.createdAt.getTime()) {
        job.metadata.updatedAt = createValidDate(job.metadata.createdAt.getTime())
      }
  ...
  describe('Property 7: Reconciliation Timeline Accuracy', () => {
    ...
    it('should maintain chronological order of timeline entries', async () => {
      ...
      ), { numRuns: 3 }) // Reduced number of runs for performance
    })
    ...
    it('should provide accurate completion estimates based on timeline patterns', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (job, stabilityPeriodDays, currentStableDays) => {
          ...
          // Ensure job has valid date relationships
          const now = new Date()
          job.startDate = createValidDate(now.getTime() - (10 * 24 * 60 * 60 * 1000)) // 10 days ago
          job.maxEndDate = createValidDate(now.getTime() + (5 * 24 * 60 * 60 * 1000)) // 5 days from now
          ...
          try {
            ...
            let currentDate = createValidDate(now.getTime() - (currentStableDays * 24 * 60 * 60 * 1000))
            for (let i = 0; i < currentStableDays; i++) {
              ...
              await freshProgressTracker.recordDataUpdate(job.id, currentDate, noChanges)
              currentDate = createValidDate(currentDate.getTime() + 24 * 60 * 60 * 1000)
            }
            ...
            if (currentStableDays >= stabilityPeriodDays) {
              ...
              expect(hoursUntilCompletion).toBeLessThanOrEqual(job.config.checkFrequencyHours + 48)
            } else {
              ...
              expect(estimatedCompletion!.getTime()).toBeGreaterThan(now.getTime() - 5000) // 5 second tolerance
              
              // Should not exceed max end date (with small tolerance for timing precision)
              expect(estimatedCompletion!.getTime()).toBeLessThanOrEqual(job.maxEndDate.getTime() + 5000) // 5 second tolerance
            }
          } finally {
            // Clean up
            try {
    ...
    it('should accurately detect stability periods and provide detailed stability information', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (job, significantChangesBefore, stableDays, additionalSignificantChangesAfter) => {
          ...
          try {
            ...
            let currentDate = new Date(now.getTime() - ((significantChangesBefore + stableDays + additionalSignificantChangesAfter) * 24 * 60 * 60 * 1000))
            ...
            for (let i = 0; i < significantChangesBefore; i++) {
              const significantChanges: DataChanges = {
                ...
                membershipChange: {
                  previous: 1000 + i * 10,
                  current: 1000 + (i + 1) * 10,
                  percentChange: job.config.significantChangeThresholds.membershipPercent + 1.0 + i * 0.5 // Ensure it exceeds threshold
                },
                timestamp: currentDate,
                sourceDataDate: currentDate.toISOString().split('T')[0]
              }
            ...
            for (let i = 0; i < additionalSignificantChangesAfter; i++) {
              const significantChanges: DataChanges = {
                ...
                clubCountChange: {
                  previous: 50 + i,
                  current: 50 + i + job.config.significantChangeThresholds.clubCountAbsolute + 1, // Ensure it exceeds threshold
                  absoluteChange: job.config.significantChangeThresholds.clubCountAbsolute + 1 // Ensure it exceeds threshold
                },
                timestamp: currentDate,
                sourceDataDate: currentDate.toISOString().split('T')[0]
              }
            ...
            const timeline = await freshProgressTracker.getReconciliationTimeline(job.id)
            const stabilityInfo = freshProgressTracker.getStabilityPeriodInfo(timeline, job.config.stabilityPeriodDays)
            const finalizationCheck = await freshProgressTracker.isReadyForFinalization(job.id)
            ...
            if (additionalSignificantChangesAfter > 0) {
              ...
              if (stableDays > 0) {
                ...
                expect(stabilityInfo.stabilityStartDate!.getTime()).toBe(stabilityStartDate.getTime())
              }
            }

            // Property 2: Last significant change date should be correctly identified
            if (significantChangesBefore > 0 || additionalSignificantChangesAfter > 0) {
              ...
              if (additionalSignificantChangesAfter > 0) {
                ...
                const expectedLastChangeDate = new Date(stabilityStartDate.getTime() + (stableDays * 24 * 60 * 60 * 1000))
                expect(stabilityInfo.lastSignificantChangeDate!.getTime()).toBeGreaterThanOrEqual(expectedLastChangeDate.getTime())
              } else if (significantChangesBefore > 0) {
                // Last significant change should be before the stability period
                expect(stabilityInfo.lastSignificantChangeDate!.getTime()).toBeLessThan(stabilityStartDate.getTime())
              }
            }

            // Property 3: Stability period progress should be correctly calculated
            const expectedProgress = Math.min(stabilityInfo.consecutiveStableDays / job.config.stabilityPeriodDays, 1.0)
            expect(stabilityInfo.stabilityPeriodProgress).toBeCloseTo(expectedProgress, 2)

            // Property 4: Finalization readiness should be based on stability period
            if (stabilityInfo.consecutiveStableDays >= job.config.stabilityPeriodDays) {
              expect(finalizationCheck.isReady).toBe(true)
              expect(finalizationCheck.reason).toContain('Stability period met')
            } else {
              // Check if max end date is reached
              const now = new Date()
              if (now >= job.maxEndDate) {
                expect(finalizationCheck.isReady).toBe(true)
                expect(finalizationCheck.reason).toContain('Maximum reconciliation period reached')
              } else {
                expect(finalizationCheck.isReady).toBe(false)
                expect(finalizationCheck.reason).toContain('more consecutive stable days')
              }
            }

            // Property 5: Timeline status should reflect stability period information
            expect(timeline.status.daysStable).toBe(stabilityInfo.consecutiveStableDays)
            
            if (job.status === 'completed') {
              // If job is already completed, timeline phase should be completed
              expect(timeline.status.phase).toBe('completed')
            } else if (stabilityInfo.consecutiveStableDays >= job.config.stabilityPeriodDays) {
              expect(timeline.status.phase).toBe('finalizing')
              expect(timeline.status.message).toContain('Stability period met')
            ...
            expect(stats.stabilityPeriod.consecutiveStableDays).toBe(stabilityInfo.consecutiveStableDays)
            expect(stats.stabilityPeriod.isInStabilityPeriod).toBe(stabilityInfo.isInStabilityPeriod)
            expect(stats.stabilityPeriod.stabilityPeriodProgress).toBeCloseTo(stabilityInfo.stabilityPeriodProgress, 2)
            expect(stats.stabilityPeriod.requiredStabilityDays).toBe(job.config.stabilityPeriodDays)
