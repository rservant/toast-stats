  ...
  try {
    ...
    if (!scenario.name || !scenario.description || !scenario.districtId || !scenario.targetMonth) {
      return res.status(400).json({
        success: false,
        error: 'Missing required scenario fields: name, description, districtId, targetMonth'
      })
    }
...
router.post('/test-data/property-tests/:property', async (req: Request, res: Response) => {
  try {
    ...
    const testCases = testDataGenerator.generatePropertyTestCases(property, count)
    
    logger.info('Property test cases generated', { property, count: testCases.length })
...
router.post('/replay/sessions', async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !description || !job || !timeline || !dataSequence) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields: name, description, job, timeline, dataSequence'
      })
    }
    
    const session = replayEngine.createReplaySession(name, description, job, timeline, dataSequence)
