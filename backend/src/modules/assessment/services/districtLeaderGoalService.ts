...
export async function getGoalById(
  id: string
): Promise<DistrictLeaderGoal | null> {
  if (!id) {
 ...
 */
export function validateGoal(goal: Partial<DistrictLeaderGoal>): {
  valid: boolean
  errors: string[]
} {
  const errors: string[] = []
  ...
  if (!goal.district_number || goal.district_number <= 0)
    errors.push('Invalid district_number')
  ...
  if (
    !goal.status ||
    !['in_progress', 'completed', 'overdue'].includes(goal.status)
  )
