...
export async function loadConfig(
  ...
  try {
    ...
    if (
      config.district_number !== districtNumber ||
      config.program_year !== programYear
    ) {
      throw new Error(
  ...
  try {
    fileWatcher = fsSync.watch(configPath, (eventType: string) => {
      if (eventType === 'change') {
        invalidateAllCache()
        console.log(
          '[Assessment Config] Configuration file changed, cache invalidated'
        )
      }
    })
  } catch (err) {
...
export function validateConfig(config: DistrictConfig): string[] {
  ...
  if (!config.year_end_targets) {
    ...
    if (
      typeof config.year_end_targets.membership_growth !== 'number' ||
      config.year_end_targets.membership_growth <= 0
    ) {
      errors.push(
        'year_end_targets.membership_growth must be a positive number'
      )
    }
    if (
      typeof config.year_end_targets.club_growth !== 'number' ||
      config.year_end_targets.club_growth <= 0
    ) {
      errors.push('year_end_targets.club_growth must be a positive number')
    }
    if (
      typeof config.year_end_targets.distinguished_clubs !== 'number' ||
      config.year_end_targets.distinguished_clubs <= 0
    ) {
      errors.push(
        'year_end_targets.distinguished_clubs must be a positive number'
      )
    }
  }

  if (
    !Array.isArray(config.recognition_levels) ||
    config.recognition_levels.length === 0
  ) {
    ...
    for (const rl of config.recognition_levels) {
      ...
      if (
        typeof rl.membershipPaymentsTarget !== 'number' ||
        rl.membershipPaymentsTarget <= 0
      ) {
        errors.push(
          `recognition_level ${rl.level}: membershipPaymentsTarget must be a positive number`
        )
      }
      if (typeof rl.paidClubsTarget !== 'number' || rl.paidClubsTarget <= 0) {
        errors.push(
          `recognition_level ${rl.level}: paidClubsTarget must be a positive number`
        )
      }
      if (
        typeof rl.distinguishedClubsTarget !== 'number' ||
        rl.distinguishedClubsTarget <= 0
      ) {
        errors.push(
          `recognition_level ${rl.level}: distinguishedClubsTarget must be a positive number`
        )
      }
    }
  }
  ...
  if (
    typeof config.csp_submission_target !== 'number' ||
    config.csp_submission_target <= 0
  ) {
    errors.push('csp_submission_target must be a positive number')
  }

  if (
    typeof config.csp_to_distinguished_clubs_ratio !== 'number' ||
    config.csp_to_distinguished_clubs_ratio < 0 ||
    config.csp_to_distinguished_clubs_ratio > 1
  ) {
    errors.push(
      'csp_to_distinguished_clubs_ratio must be a number between 0 and 1'
    )
  }
