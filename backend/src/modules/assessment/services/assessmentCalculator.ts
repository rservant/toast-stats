...
export function calculateGoal3(
  ...
  if (
    assessment.distinguished_clubs_ytd !== null &&
    assessment.distinguished_clubs_ytd !== undefined
  ) {
    ...
    actual = roundToNearest(
      assessment.csp_submissions_ytd * config.csp_to_distinguished_clubs_ratio
    )
    const delta = actual - cumulativeTarget
    status = determineStatus(actual, cumulativeTarget)
 ...
 */
export function getRecognitionLevelBreakdown(config: DistrictConfig): {
  level: string
  membership: number
  clubs: number
