...
describe('configService', () => {
  ...
  describe('validateConfig', () => {
    ...
    it('should reject missing year_end_targets', () => {
      const invalid = { ...mockConfig, year_end_targets: undefined } as unknown as DistrictConfig;
      const errors = validateConfig(invalid);
    ...
    it('should reject invalid membership growth target', () => {
      const invalid = {
        ...mockConfig,
        year_end_targets: { ...mockConfig.year_end_targets, membership_growth: -1 },
      };
      const errors = validateConfig(invalid);
    ...
    it('should reject invalid CSP ratio (out of range)', () => {
      ...
      const errors = validateConfig(invalid);
      expect(errors.some((e) => e.includes('csp_to_distinguished_clubs_ratio'))).toBe(true);
    });
    ...
    it('should accept CSP ratio at boundaries', () => {
      ...
      expect(errors1.some((e) => e.includes('csp_to_distinguished_clubs_ratio'))).toBe(false);

      const config2 = { ...mockConfig, csp_to_distinguished_clubs_ratio: 1 };
      const errors2 = validateConfig(config2);
      expect(errors2.some((e) => e.includes('csp_to_distinguished_clubs_ratio'))).toBe(false);
    });
  });
});
