...
describe('monthlyTargetService', () => {
  describe('deriveMonthlyTargets', () => {
    ...
    it('should derive recognition level targets', () => {
      ...
      expect(target.recognition_level_targets[0].membershipPaymentsTarget).toBe(3); // 30 / 12 ≈ 2.5 → 3
      expect(target.recognition_level_targets[0].paidClubsTarget).toBe(0); // 3 / 12 = 0.25 → 0
      expect(target.recognition_level_targets[0].distinguishedClubsTarget).toBe(1); // 6 / 12 = 0.5 → 0 or 1
    });

    it('should handle rounding correctly', () => {
      const customConfig = {
        ...mockConfig,
        year_end_targets: { ...mockConfig.year_end_targets, membership_growth: 100 },
      };
  ...
  describe('getMonthlyTarget', () => {
    ...
    it('should retrieve target by month number', () => {
      ...
      expect(target1.membership_growth_target).toBe(target2.membership_growth_target);
    });
  ...
  describe('validateMonthlyTargets', () => {
    ...
    it('should reject negative targets', () => {
      ...
      expect(errors.some((e) => e.includes('Membership growth target'))).toBe(true);
    });
    ...
    it('should reject missing recognition level targets', () => {
      ...
      const errors = validateMonthlyTargets(invalidTarget, mockConfig);
      expect(errors.some((e) => e.includes('Recognition level targets'))).toBe(true);
    });
  });
});
