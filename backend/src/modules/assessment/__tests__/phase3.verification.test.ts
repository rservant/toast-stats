...
import { calculateAllGoals } from '../services/assessmentCalculator'
import {
...
describe('Phase 3 Verification - Assessment Calculator vs. expected targets', () => {
  ...
  data.monthly_data.forEach((monthData: MonthlyData) => {
    it(`calculates correct goals for ${monthData.month}`, () => {
      ...
      const expectedGoal1Status =
        expectedGoal1Actual >= expectedGoal1Target ? 'On Track' : 'Off Track'
      ...
      const expectedGoal2Status =
        expectedGoal2Actual >= expectedGoal2Target ? 'On Track' : 'Off Track'
      ...
      if (
        monthData.distinguished_clubs_ytd !== null &&
        monthData.distinguished_clubs_ytd !== undefined
      ) {
        expectedGoal3Actual = monthData.distinguished_clubs_ytd
      } else if (monthData.csp_submissions_ytd > 0) {
        expectedGoal3Actual = Math.round(
          monthData.csp_submissions_ytd *
            config.csp_to_distinguished_clubs_ratio
        )
      } else {
        expectedGoal3Actual = 0
      }

      const expectedGoal3Status =
        expectedGoal3Actual >= expectedGoal3Target
          ? 'On Track'
          : expectedGoal3Actual === 0
            ? 'Pending Data'
            : 'Off Track'
