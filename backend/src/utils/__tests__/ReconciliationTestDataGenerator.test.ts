...
describe('ReconciliationTestDataGenerator', () => {
  ...
  describe('generateTestData', () => {
    ...
    it('should generate gradual_growth pattern with changes', () => {
      ...
      const membershipChanges = testData.expectedChanges.filter(c => c.membershipChange)
      expect(membershipChanges.length).toBeGreaterThan(0)
      
      membershipChanges.forEach(change => {
        expect(change.membershipChange!.current).toBeGreaterThan(change.membershipChange!.previous)
    ...
    it('should generate sudden_change pattern with significant change', () => {
      ...
      expect(testData.metadata.expectedOutcome).toBe('extended_due_to_late_change')
    ...
    it('should generate volatile_changes pattern with frequent changes', () => {
      ...
      expect(testData.metadata.expectedOutcome).toBe('extended_due_to_volatility')
    ...
    it('should generate late_stabilization pattern', () => {
      ...
      expect(testData.metadata.expectedOutcome).toBe('completed_after_extension')
    })
    ...
    it('should generate club_changes pattern', () => {
      ...
      expect(testData.metadata.expectedOutcome).toBe('completed_with_club_changes')
      
      // Should have club count changes
      const clubChanges = testData.expectedChanges.filter(c => c.clubCountChange)
    ...
    it('should generate distinguished_changes pattern', () => {
      ...
      expect(testData.metadata.expectedOutcome).toBe('completed_with_distinguished_changes')
      
      // Should have distinguished changes
      const distinguishedChanges = testData.expectedChanges.filter(c => c.distinguishedChange)
    ...
    it('should throw error for unknown pattern', () => {
      expect(() => generator.generateTestData('unknown_pattern')).toThrow('Test data pattern not found: unknown_pattern')
    })
    ...
    it('should generate different data with different seeds', () => {
      ...
      const different = testData1.districtData[0].membership.total !== testData2.districtData[0].membership.total ||
                       testData1.districtData[0].clubs.total !== testData2.districtData[0].clubs.total
  ...
  describe('generateBatchTestData', () => {
    it('should generate multiple test data sets', () => {
      ...
      const stableCount = testSets.filter(s => s.metadata.pattern === 'stable_no_changes').length
      const growthCount = testSets.filter(s => s.metadata.pattern === 'gradual_growth').length
    ...
    it('should handle invalid patterns gracefully', () => {
      const patterns = ['stable_no_changes', 'invalid_pattern', 'gradual_growth']
      ...
      expect(testSets.every(s => s.metadata.pattern !== 'invalid_pattern')).toBe(true)
    })
  ...
  describe('generatePropertyTestCases', () => {
    it('should generate change_detection_accuracy test cases', () => {
      const testCases = generator.generatePropertyTestCases('change_detection_accuracy', 10)
    ...
    it('should generate stability_period_calculation test cases', () => {
      const testCases = generator.generatePropertyTestCases('stability_period_calculation', 5)
    ...
    it('should generate configuration_validation test cases', () => {
      const testCases = generator.generatePropertyTestCases('configuration_validation', 8)
    ...
    it('should return empty array for unknown property', () => {
      const testCases = generator.generatePropertyTestCases('unknown_property', 5)
      expect(testCases).toHaveLength(0)
    })

    it('should generate unique seeds for each test case', () => {
      const testCases = generator.generatePropertyTestCases('change_detection_accuracy', 20)
  ...
  describe('generateEdgeCases', () => {
    ...
    it('should generate zero membership edge case correctly', () => {
      const edgeCases = generator.generateEdgeCases()
      const zeroCase = edgeCases.find(ec => ec.metadata.pattern === 'edge_case_zero_membership')!
    ...
    it('should generate single club edge case correctly', () => {
      const edgeCases = generator.generateEdgeCases()
      const singleCase = edgeCases.find(ec => ec.metadata.pattern === 'edge_case_single_club')!
    ...
    it('should generate max size edge case correctly', () => {
      const edgeCases = generator.generateEdgeCases()
      const maxCase = edgeCases.find(ec => ec.metadata.pattern === 'edge_case_max_size')!
    ...
    it('should generate all distinguished edge case correctly', () => {
      const edgeCases = generator.generateEdgeCases()
      const allDistCase = edgeCases.find(ec => ec.metadata.pattern === 'edge_case_all_distinguished')!
    ...
    it('should generate no distinguished edge case correctly', () => {
      const edgeCases = generator.generateEdgeCases()
      const noDistCase = edgeCases.find(ec => ec.metadata.pattern === 'edge_case_no_distinguished')!
  ...
  describe('seeded random generation', () => {
    it('should produce consistent results with same seed', () => {
      ...
      results.slice(1).forEach(result => {
        expect(result.districtData[0].membership.total).toBe(firstResult.districtData[0].membership.total)
        expect(result.districtData[0].clubs.total).toBe(firstResult.districtData[0].clubs.total)
        expect(result.config.maxReconciliationDays).toBe(firstResult.config.maxReconciliationDays)
      })
    })
    ...
    it('should produce different results with different seeds', () => {
      ...
      const membershipTotals = results.map(r => r.districtData[0].membership.total)
