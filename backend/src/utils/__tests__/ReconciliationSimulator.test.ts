...
describe('ReconciliationSimulator', () => {
  ...
  describe('initialization', () => {
    ...
    it('should have valid default scenario configurations', () => {
      ...
      scenarios.forEach(scenario => {
        ...
        expect(['completed', 'extended', 'timeout', 'failed']).toContain(scenario.expectedOutcome)
  ...
  describe('simulateScenario', () => {
    ...
    it('should throw error for non-existent scenario', async () => {
      await expect(simulator.simulateScenario('non_existent')).rejects.toThrow('Scenario not found: non_existent')
    })
  })
  ...
  describe('runBatchSimulation', () => {
    it('should run multiple scenarios successfully', async () => {
      ...
      results.forEach(result => {
        ...
        expect(['completed', 'extended', 'timeout', 'failed']).toContain(result.actualOutcome)
      })
    })

    it('should handle scenario failures gracefully', async () => {
      const scenarioNames = ['stable_quick', 'non_existent', 'gradual_stabilization']
      const results = await simulator.runBatchSimulation(scenarioNames)
      
      // Should return results for valid scenarios only
      expect(results).toHaveLength(2)
      expect(results.map(r => r.scenario.name)).toEqual(['stable_quick', 'gradual_stabilization'])
    })
  ...
  describe('change detection simulation', () => {
    it('should detect membership changes correctly', async () => {
      ...
      const membershipChanges = result.changeEvents.filter(c => c.membershipChange)
    ...
    it('should calculate significance correctly', async () => {
      ...
      const significantChanges = result.timeline.entries.filter(e => e.isSignificant)
  ...
  describe('timeline generation', () => {
    ...
    it('should track status progression correctly', async () => {
      ...
      expect(result.timeline.status.phase).toMatch(/monitoring|stabilizing|completed|failed/)
  ...
  describe('metrics calculation', () => {
    it('should calculate accurate metrics', async () => {
      ...
      expect(result.metrics.significantChanges).toBeLessThanOrEqual(result.metrics.totalChanges)
      ...
      const manualSignificantCount = result.timeline.entries.filter(e => e.isSignificant).length
