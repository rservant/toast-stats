...
import { CircuitBreaker, CircuitState, CircuitBreakerError, CircuitBreakerManager } from '../CircuitBreaker.js'
...
describe('CircuitBreaker', () => {
  ...
  describe('basic functionality', () => {
    ...
    it('should reject requests immediately when OPEN', async () => {
      ...
      await expect(breaker.execute(fastOperation)).rejects.toThrow(CircuitBreakerError)
    ...
    it('should transition to HALF_OPEN after recovery timeout', async () => {
      ...
      // Open the circuit
      await expect(breaker.execute(failingOperation)).rejects.toThrow('test error')
      expect(breaker.getStats().state).toBe(CircuitState.OPEN)
  ...
  describe('error filtering', () => {
    it('should not count filtered errors as failures', async () => {
      ...
      const clientErrorOperation = vi.fn().mockRejectedValue(new Error('client error'))
      const serverErrorOperation = vi.fn().mockRejectedValue(new Error('server error'))
      
      // Client error should not count
      await expect(breaker.execute(clientErrorOperation)).rejects.toThrow('client error')
      expect(breaker.getStats().state).toBe(CircuitState.CLOSED)
      expect(breaker.getStats().failureCount).toBe(0)
      
      // Server error should count
      await expect(breaker.execute(serverErrorOperation)).rejects.toThrow('server error')
      expect(breaker.getStats().state).toBe(CircuitState.CLOSED)
  ...
  describe('factory methods', () => {
    it('should create dashboard circuit breaker with appropriate settings', () => {
      const breaker = CircuitBreaker.createDashboardCircuitBreaker('test-dashboard')
      const stats = breaker.getStats()
...
describe('CircuitBreakerManager', () => {
  ...
  it('should reset all circuit breakers', async () => {
    ...
    const breaker = manager.getCircuitBreaker('reset-test', { failureThreshold: 1 })
    
    // Open the circuit
    await expect(breaker.execute(() => Promise.reject(new Error('test')))).rejects.toThrow()
    expect(breaker.getStats().state).toBe(CircuitState.OPEN)
