...
async function clearRankingsCache() {
  try {
    ...
    console.log(`   - Cache size: ${(statsBefore.cacheSize / 1024 / 1024).toFixed(2)} MB`)
    console.log('')
    ...
    if (clearedIncompatible > 0) {
      console.log(`   ✅ Cleared ${clearedIncompatible} incompatible cache entries`)
    } else {
    ...
    console.log(`   - Cache size: ${(statsAfter.cacheSize / 1024 / 1024).toFixed(2)} MB`)
    console.log('')
