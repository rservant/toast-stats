...
*For any* valid ISO date string provided as `endDate`, the `getSnapshotForDate` helper SHALL return the snapshot with that exact `snapshot_id`, or return an error if no such snapshot exists.

**Validates: Requirements 1.1, 2.1, 3.1, 4.1, 5.1**

### Property 2: Error Response for Non-Existent Snapshots

*For any* date string that does not correspond to an existing snapshot, the helper SHALL return an error object with:
...
*For any* call to `getSnapshotForDate` without an `endDate` parameter (undefined), the helper SHALL return the same snapshot as `snapshotStore.getLatestSuccessful()`.
