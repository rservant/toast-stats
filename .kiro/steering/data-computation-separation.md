...
**Applies to:** All backend and collector-cli code  
...
- Consolidate all data computation in the collector-cli pipeline
...
### Collector CLI (`packages/collector-cli/`)
  ...
  if (!data) {
    ...
    return {
      error: {
        ...
        details: 'Run collector-cli to generate this data',
      },
    }
  }
...
## 6. Required Patterns in Collector CLI

### 6.1 Complete Data Pipeline

The collector-cli MUST generate ALL data files required by the backend:
...
The following files MUST be generated by collector-cli for each snapshot:
...
Any existing computation in the backend MUST be migrated to collector-cli:

1. Identify computation logic in backend
2. Move computation to `packages/analytics-core/` (shared package)
3. Call from collector-cli pipeline
...
2. Add generation step to collector-cli pipeline
...
The `packages/shared-contracts/` package defines the **data contract** between collector-cli and backend:
...
```typescript
// collector-cli: Writing pre-computed files
...
// ❌ FORBIDDEN - Defining file types in collector-cli only
// packages/collector-cli/src/types/myNewType.ts
export interface MyNewFileType { ... }
...
All computation time MUST be spent in the collector-cli pipeline.
...
// Rankings not found
{
  ...
  details: 'The snapshot does not contain all-districts-rankings data. Run collector-cli to generate rankings.',
}
...
// Analytics not found
{
  ...
  details: 'Run collector-cli compute-analytics to generate analytics files.',
}
