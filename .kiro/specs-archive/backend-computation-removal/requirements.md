...
1. Extend the collector-cli pre-computation pipeline to generate all required analytics files
...
- **Pre-Computed Analytics**: Analytics computed during the collector-cli pipeline and stored as JSON files
...
**User Story:** As a system operator, I want membership analytics to be pre-computed by collector-cli, so that the backend can serve them without on-demand computation.

#### Acceptance Criteria

1. WHEN the collector-cli compute-analytics command runs, THE System SHALL generate a `membership-analytics.json` file for each district
...
1. WHEN the collector-cli compute-analytics command runs, THE System SHALL generate club trend data for each club in each district
...
1. WHEN the collector-cli compute-analytics command runs, THE System SHALL generate a `vulnerable-clubs.json` file for each district
...
1. WHEN the collector-cli compute-analytics command runs, THE System SHALL generate a `leadership-insights.json` file for each district
...
1. WHEN the collector-cli compute-analytics command runs, THE System SHALL generate a `distinguished-club-analytics.json` file for each district
...
1. WHEN the collector-cli compute-analytics command runs, THE System SHALL generate a `year-over-year.json` file for each district
...
1. WHEN the collector-cli compute-analytics command runs, THE System SHALL generate a `performance-targets.json` file for each district
...
**User Story:** As a system maintainer, I want the AnalyticsEngine and analytics modules removed from the backend, so that there is a single source of truth for analytics computation in collector-cli.
...
1. WHEN pre-computed files are missing, THE Backend SHALL return informative 404 errors with instructions to run collector-cli
2. THE error messages SHALL indicate which collector-cli command needs to be run
3. THE collector-cli compute-analytics command SHALL generate all required pre-computed files in a single run
