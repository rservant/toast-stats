...
2. Move ranking computation from SnapshotBuilder to scraper-cli transform command
3. Update scraper-cli to generate time-series index files during compute-analytics
4. Update scraper-cli transform command to compute rankings
...
- **transform**: The scraper-cli command that converts raw CSV to snapshot JSON files
- **compute-analytics**: The scraper-cli command that generates pre-computed analytics files
- **On-Demand_Computation**: Computing data at API request time (FORBIDDEN by steering document)
- **Pre-Computed_Data**: Data computed during the scraper-cli pipeline and stored as JSON files
...
### Requirement 4: Generate Time-Series Index in scraper-cli

**User Story:** As a system operator, I want time-series index data to be generated by scraper-cli, so that the backend can serve it without on-demand computation.

#### Acceptance Criteria

1. WHEN the scraper-cli compute-analytics command runs, THE System SHALL generate time-series data points for each district
...
### Requirement 5: Generate Rankings in scraper-cli Transform Command
...
1. WHEN the scraper-cli transform command runs with all-districts data, THE System SHALL compute Borda count rankings
...
**User Story:** As a developer, I want time-series computation logic in a shared package, so that scraper-cli can use it without code duplication.
...
**User Story:** As a developer, I want ranking computation logic in a shared package, so that scraper-cli can use it without code duplication.
...
2. THE scraper-cli compute-analytics command SHALL regenerate time-series indexes from existing snapshots
...
3. THE program year summary SHALL be pre-computed by scraper-cli and stored in the index files
...
**User Story:** As a developer, I want time-series data types defined in shared-contracts, so that both scraper-cli and backend use the same validated types.
...
6. THE scraper-cli SHALL import time-series types from shared-contracts
...
2. THE distinguishedProjection SHALL be pre-computed by scraper-cli and stored in analytics files
3. THE core.ts route SHALL NOT compute overallRank by sorting rankings
4. THE overallRank SHALL be pre-computed by scraper-cli and stored in rankings files
