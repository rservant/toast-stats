...
2. **Extend collector-cli** - Generate time-series indexes and rankings during pipeline
...
### 3. TimeSeriesIndexWriter (collector-cli)
...
### 4. Updated AnalyticsComputeService (collector-cli)

Extended to generate time-series indexes alongside analytics.

```typescript
// packages/collector-cli/src/services/AnalyticsComputeService.ts
...
### 5. Updated TransformService (collector-cli)
...
_For any_ time-series index file generated by collector-cli, the file SHALL validate against the TimeSeriesDataPoint and ProgramYearIndexFile Zod schemas defined in shared-contracts.

**Validates: Requirements 10.1, 10.2, 10.3, 10.4, 10.5, 13.1, 13.2, 13.3, 13.4, 13.5**

## Error Handling

### Collector-CLI Errors
...
| Rankings file not found    | 404       | RANKINGS_NOT_FOUND      | Run collector-cli transform |
| Schema version mismatch    | 500       | SCHEMA_VERSION_MISMATCH | Re-run collector-cli        |
