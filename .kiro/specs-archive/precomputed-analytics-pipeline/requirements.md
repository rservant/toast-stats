...
- **Pre_Computed_Analytics**: JSON files containing analytics results computed during scraper runs rather than on API request
...
**User Story:** As a system operator, I want analytics to be computed during local scraper runs, so that the backend can serve data without heavy memory usage.

#### Acceptance Criteria

1. WHEN the Scraper_CLI completes a scrape operation, THE Scraper_CLI SHALL transform raw CSV data into snapshots using the same transformation logic as the Backend
2. WHEN snapshots are created, THE Scraper_CLI SHALL compute analytics for each district using the same algorithms as the Analytics_Engine
3. WHEN computing analytics, THE Scraper_CLI SHALL generate membership trends, club health scores, distinguished club projections, division/area comparisons, and year-over-year metrics
4. THE Pre_Computed_Analytics SHALL produce identical results to the current Analytics_Engine for the same input data
5. IF analytics computation fails for a district, THEN THE Scraper_CLI SHALL log the error and continue processing remaining districts
6. WHEN analytics are computed, THE Scraper_CLI SHALL store them in an `analytics/` subdirectory within the snapshot directory
7. THE data pipeline SHALL be: raw CSV → snapshot → pre-computed analytics (all in Scraper_CLI)

### Requirement 2: Snapshot Creation in Scraper CLI

**User Story:** As a system operator, I want snapshots created during scraper runs, so that the full data pipeline runs locally before upload.

#### Acceptance Criteria

1. THE Scraper_CLI SHALL provide a `transform` command that converts raw CSV files into snapshot format
2. WHEN transforming raw CSVs, THE Scraper_CLI SHALL use the same DataTransformationService logic as the Backend
3. THE Scraper_CLI SHALL store snapshots in the same directory structure as the Backend expects: `CACHE_DIR/snapshots/{date}/`
4. WHEN a snapshot is created, THE Scraper_CLI SHALL write district JSON files, metadata.json, and manifest.json
5. THE `scrape` command SHALL optionally run transformation automatically with a `--transform` flag
6. THE data transformation logic SHALL be extracted into a shared package usable by both Scraper_CLI and Backend
...
1. THE Scraper_CLI SHALL store pre-computed analytics in the following structure:
   ```
   ...
   ```
2. WHEN writing analytics files, THE Scraper_CLI SHALL include a schema version and computation timestamp in each file
...
5. THE Scraper_CLI SHALL write an analytics manifest file listing all generated analytics files with their checksums
...
**User Story:** As a system operator, I want analytics to be updated incrementally, so that scraper runs complete quickly without recomputing everything.

#### Acceptance Criteria

1. WHEN the Scraper_CLI runs, THE Scraper_CLI SHALL check if analytics already exist for the target date
2. IF analytics exist and the underlying snapshot data has not changed, THEN THE Scraper_CLI SHALL skip analytics computation for that district
3. WHEN new snapshot data is scraped, THE Scraper_CLI SHALL recompute only the affected analytics files
4. THE Scraper_CLI SHALL provide a `--force-analytics` flag to recompute all analytics regardless of existing files
5. WHEN checking for changes, THE Scraper_CLI SHALL compare snapshot checksums to determine if recomputation is needed
...
1. THE Scraper_CLI SHALL provide an `upload` command to sync local snapshots and analytics to Google Cloud Storage
2. WHEN uploading, THE Scraper_CLI SHALL upload both snapshot data and pre-computed analytics files
3. THE Scraper_CLI SHALL support incremental uploads, only uploading files that have changed
4. IF upload fails for any file, THEN THE Scraper_CLI SHALL report the failure and continue with remaining files
5. WHEN upload completes, THE Scraper_CLI SHALL output a summary of uploaded files and any errors

### Requirement 7: Analytics Computation Modules

**User Story:** As a developer, I want analytics computation logic to be shared between scraper-cli and backend, so that results are consistent.

#### Acceptance Criteria

1. THE Analytics computation logic SHALL be extracted into a shared package that both Scraper_CLI and Backend can use
...
1. THE Scraper_CLI SHALL provide a `compute-analytics` command that computes analytics for existing snapshots
2. WHEN the `compute-analytics` command is invoked with a date, THE Scraper_CLI SHALL compute analytics for that snapshot
...
5. IF the specified snapshot does not exist, THEN THE Scraper_CLI SHALL return an error with exit code 2
...
2. WHEN writing analytics files, THE Scraper_CLI SHALL include metadata: schemaVersion, computedAt, snapshotDate, districtId
...
1. WHEN analytics computation fails, THE Scraper_CLI SHALL log the district ID, error message, and stack trace
2. THE Scraper_CLI SHALL provide a `--verbose` flag for detailed computation logging
...
3. WHEN the Scraper_CLI generates pre-computed analytics locally, THE Backend SHALL be able to serve them immediately without any upload step
4. THE Scraper_CLI `compute-analytics` command SHALL work with locally scraped data without requiring cloud connectivity
