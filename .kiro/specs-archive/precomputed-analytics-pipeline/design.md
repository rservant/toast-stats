...
```mermaid
flowchart TB
    subgraph "Local Machine (collector-cli)"
...
2. **Shared computation modules**: Analytics algorithms are extracted into a shared package (`@toastmasters/analytics-core`) used by both collector-cli and backend (for validation only).
...
4. **Incremental updates**: The collector-cli tracks checksums to avoid recomputing unchanged data.
...
This new package contains the analytics computation logic extracted from the backend, making it usable by both collector-cli and backend.
...
### 3. Pre-Computed Analytics Writer (collector-cli)

**Location**: `packages/collector-cli/src/services/AnalyticsWriter.ts`
...
### 5. CLI Commands (collector-cli)

**New commands added to collector-cli:**
...
_For any_ pre-computed analytics file written by the Collector_CLI, the file SHALL contain metadata with all required fields: `schemaVersion`, `computedAt`, `snapshotDate`, `districtId`, and `checksum`.

**Validates: Requirements 3.2, 9.2**

### Property 3: Analytics File Size Constraint

_For any_ pre-computed analytics file generated by the Collector_CLI, the file size SHALL be less than 100KB.
...
_For any_ snapshot with unchanged checksum compared to a previous run, the Collector_CLI SHALL skip analytics recomputation and preserve existing analytics files.
...
_For any_ snapshot created by the Collector*CLI, the snapshot directory SHALL contain `metadata.json`, `manifest.json`, and at least one `district*{id}.json` file.
...
packages/collector-cli/src/__tests__/
