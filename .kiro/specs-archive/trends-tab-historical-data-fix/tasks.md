...
Fix three bugs in the Trends tab: (1) sparse time-series data returning empty membership trends, (2) payments trend hook using wrong program year, (3) YoY comparison using same snapshot for both years + backend sending wrong change field type. Changes span backend, frontend, and collector-cli.
...
- [x] 4. Fix YoY comparison to use previous year snapshot
  - [x] 4.1 Load previous year snapshot in AnalyticsComputeService
    - In `packages/collector-cli/src/services/AnalyticsComputeService.ts`, in the `computeDistrictAnalytics` method, after loading the current snapshot:
...
- The collector-cli change (task 4.1) means existing pre-computed YoY files will need to be regenerated by re-running `compute-analytics` for snapshots that have a previous year snapshot available.
