...
- **Pre_Computed_Analytics**: Analytics files generated by collector-cli and stored on disk, read by the backend without any computation
...
**User Story:** As a system operator, I want the collector-cli `compute-analytics` command to produce correct year-over-year data after the `findSnapshotForDate` fix, so that the backend serves accurate pre-computed YoY comparisons.

#### Acceptance Criteria

1. WHEN the collector-cli `compute-analytics` command runs and no previous year snapshot exists on disk for a district, THE command SHALL produce a pre-computed analytics file with `yearOverYear.dataAvailable` set to `false`
2. WHEN the collector-cli `compute-analytics` command runs and a valid previous year snapshot exists on disk for a district, THE command SHALL produce a pre-computed analytics file with `yearOverYear.dataAvailable` set to `true` and accurate comparison metrics
