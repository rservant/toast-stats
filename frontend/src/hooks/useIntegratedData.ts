...
import { useMembershipHistory, useDistrictStatistics } from './useMembershipData';
...
export function useEnhancedMembershipData(
  ...
  const { data: membershipHistory, isLoading: isLoadingHistory, error: historyError } = 
  ...
  const { data: dailyReportsData, isLoading: isLoadingReports, error: reportsError } = 
...
export function useSignificantEvents(
  ...
  const { data: dailyReportsData, isLoading, error } = 
    useDailyReports(districtId, startDate, endDate);
