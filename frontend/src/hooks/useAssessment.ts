...
export const useAssessment = () => {
  ...
  const computeMonthlySummary = async (
    ...
    try {
      ...
      const responses = await Promise.all(
        datesInMonth.map(d =>
          apiClient
            .get(`/districts/${districtId}/data/${d}`)
            .then(r => r.data)
            .catch(() => null)
        )
      )
      ...
      const latestData: DistrictDataResponse | null = latest
        ? await apiClient
            .get(`/districts/${districtId}/data/${latest}`)
            .then(r => r.data)
            .catch(() => null)
        : null
      ...
      if (latestData && Array.isArray(latestData.clubPerformance)) {
        ...
        totalMembership = latestData.clubPerformance.reduce(
          (acc: number, club: ClubData) =>
            acc + extractMembershipFromClub(club),
          0
        )
  ...
  const generateAssessment = async (payload: {
    districtId: string
    programYearStart: string
    programYearEnd: string
    month: string
  }) => {
    ...
    console.log(
      'useAssessment.generateAssessment - transformed payload:',
      backendPayload
    )
  ...
  const fetchAssessment = async (
    ...
    try {
      const resp = await apiClient.get(
        `/assessment/monthly/${districtId}/${encodeURIComponent(programYear)}/${encodeURIComponent(month)}`
      )
      return resp.data
    } catch (err: unknown) {
