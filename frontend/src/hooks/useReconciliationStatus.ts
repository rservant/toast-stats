 ...
 */
export const useReconciliationStatus = (districtId: string, targetMonth: string) => {
  return useQuery({
    queryKey: ['reconciliation-status', districtId, targetMonth],
    queryFn: async (): Promise<DataStatusResponse> => {
      const response = await apiClient.get(`/reconciliation/status/${districtId}/${targetMonth}`);
