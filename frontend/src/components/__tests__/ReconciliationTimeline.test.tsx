...
import { ReconciliationTimeline, ReconciliationTimelineEntry } from '../ReconciliationTimeline';
...
describe('ReconciliationTimeline', () => {
  ...
  it('renders timeline with entries correctly', () => {
    render(<ReconciliationTimeline entries={mockEntries} targetMonth="2024-10" />);
  ...
  it('displays empty state when no entries provided', () => {
    ...
    expect(screen.getByText('No reconciliation activity yet for 2024-10')).toBeInTheDocument();
  });

  it('shows change summaries when showDetails is true', () => {
    render(<ReconciliationTimeline entries={mockEntries} targetMonth="2024-10" showDetails={true} />);
  ...
  it('hides change summaries when showDetails is false', () => {
    render(<ReconciliationTimeline entries={mockEntries} targetMonth="2024-10" showDetails={false} />);
    
    expect(screen.queryByText('Membership increased by 5%')).not.toBeInTheDocument();
    expect(screen.queryByText('Minor club count adjustment')).not.toBeInTheDocument();
  });
  ...
  it('handles single entry correctly', () => {
    ...
    render(<ReconciliationTimeline entries={singleEntry} targetMonth="2024-10" />);
  ...
  it('maintains accessibility standards', () => {
    ...
    expect(screen.getByRole('heading', { level: 3 })).toHaveTextContent('Reconciliation Timeline');
  ...
  it('handles entries with missing optional fields', () => {
    ...
    expect(() => {
      render(<ReconciliationTimeline entries={entriesWithMissingFields} targetMonth="2024-10" />);
    }).not.toThrow();
