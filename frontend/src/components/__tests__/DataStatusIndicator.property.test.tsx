...
const dataStatusArb = fc
  .record({
    isPreliminary: fc.boolean(),
    isFinal: fc.boolean(),
    dataCollectionDate: fc
      .integer({ min: Date.parse('2020-01-01'), max: Date.parse('2030-12-31') })
      .map(ms => new Date(ms).toISOString()),
    reconciliationStatus: fc.option(reconciliationStatusArb, {
      nil: undefined,
    }),
    lastUpdated: fc
      .integer({ min: Date.parse('2020-01-01'), max: Date.parse('2030-12-31') })
...
describe('DataStatusIndicator Property Tests', () => {
  ...
  it('Property 5.2: Data collection date is always formatted and displayed correctly', () => {
    fc.assert(
      fc.property(dataStatusArb, dataStatus => {
        const { container } = render(
          <DataStatusIndicator dataStatus={dataStatus} showDetails={true} />
        )
