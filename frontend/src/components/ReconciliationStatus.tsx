...
export const ReconciliationStatus: React.FC<ReconciliationStatusProps> = ({
    ...
    switch (status.phase) {
      case 'monitoring':
        return {
          ...
          description: 'Actively checking for data updates from Toastmasters dashboard',
  ...
  return (
    <div className={`bg-white rounded-lg border border-gray-200 p-6 ${className}`}>
      ...
      {/* Status Badge */}
      <div className="mb-6">
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border ${phaseInfo.color}`}>
          {phaseInfo.icon}
        ...
        <div className="space-y-3 text-sm">
          ...
          {status.daysStable > 0 && (
            <div>
              ...
              <div className="font-medium">
                {status.daysStable} day{status.daysStable !== 1 ? 's' : ''} without changes
              </div>
            </div>
          )}
          ...
          {status.lastChangeDate && (
            <div>
              ...
              <div className="font-medium">{formatDate(status.lastChangeDate)}</div>
            </div>
          )}

          {status.nextCheckDate && status.phase !== 'completed' && status.phase !== 'failed' && (
            <div>
              <span className="text-gray-600">Next Check:</span>
              <div className="font-medium">{formatDate(status.nextCheckDate)}</div>
            </div>
          )}
          ...
          {status.message && (
            <div className="pt-2 border-t border-gray-200">
              ...
              <div className="font-medium text-gray-900 mt-1">{status.message}</div>
            </div>
          )}
        </div>
      )}
