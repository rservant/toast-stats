...
export const ClubDetailModal: React.FC<ClubDetailModalProps> = ({ club, onClose }) => {
  if (!club) return null;
  ...
  const latestMembership = club.membershipTrend.length > 0
    ? club.membershipTrend[club.membershipTrend.length - 1].count
    : 0;

  // Get membership change
  const membershipChange = club.membershipTrend.length > 1
    ? club.membershipTrend[club.membershipTrend.length - 1].count - club.membershipTrend[0].count
    : 0;

  // Get latest DCP goals
  const latestDcpGoals = club.dcpGoalsTrend.length > 0
  ...
  const handleExport = () => {
    ...
    const maxLength = Math.max(club.membershipTrend.length, club.dcpGoalsTrend.length);
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex-1">
              <h3 className="text-2xl font-bold text-gray-900">{club.clubName}</h3>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div className="flex items-center justify-between mb-4 text-sm">
                  <div>
                    ...
                    <span className="font-semibold text-gray-900">{latestMembership} members</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Change: </span>
                    <span className={`font-semibold ${membershipChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {membershipChange >= 0 ? '+' : ''}{membershipChange} members
                    </span>
                  </div>
                </div>
                  ...
                  <svg className="w-full h-full" viewBox="0 0 800 160" preserveAspectRatio="none">
                    ...
                    <polyline
                      ...
                      points={club.membershipTrend
                        .map((point, index) => {
                          const x = (index / (club.membershipTrend.length - 1)) * 800;
                          const y = 160 - ((point.count - minMembership) / membershipRange) * 160;
                          return `${x},${y}`;
                        })
                        .join(' ')}
                    />
                ...
                <div className="flex items-center justify-between mt-2 text-xs text-gray-600">
                  ...
                  <span>{formatDate(club.membershipTrend[club.membershipTrend.length - 1].date)}</span>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <span className="font-semibold text-gray-900">{latestDcpGoals} / 10 goals</span>
                  </div>
                ...
                {/* Historical Progress */}
                <div className="space-y-2">
                  {club.dcpGoalsTrend.slice(-5).reverse().map((point, index) => (
                    <div key={index} className="flex items-center gap-3 text-sm">
                      <span className="text-gray-600 w-24">{formatDate(point.date)}</span>
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
                        <div
                          className="bg-blue-500 h-2 rounded-full"
                          style={{ width: `${(point.goalsAchieved / 10) * 100}%` }}
                        ></div>
                      </div>
                      <span className="text-gray-900 w-16 text-right">{point.goalsAchieved} / 10</span>
                    </div>
                  ))}
                </div>
              </div>
