...
> = ({ isAdmin }) => {
  ...
  const [configFormData, setConfigFormData] =
    useState<ReconciliationConfig | null>(null)
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error?.message ||
            `Failed to start reconciliation: ${response.statusText}`
        )
      }
  ...
  const isStartFormValid =
    startFormData.districtId.trim() !== '' &&
    startFormData.targetMonth.trim() !== ''
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="mt-2 space-y-1 text-sm text-gray-600">
                        <p>
                          <strong>Started:</strong> {formatDate(job.startDate)}
                        </p>
                        {job.currentDataDate && (
                          <p>
                            <strong>Current Data Date:</strong>{' '}
                            {job.currentDataDate}
                          </p>
                        )}
        ...
        <div className="tm-card">
          <div className="px-4 py-5 sm:p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <strong>Max Reconciliation Days:</strong>{' '}
                {config.maxReconciliationDays}
              </div>
              <div>
                <strong>Stability Period Days:</strong>{' '}
                {config.stabilityPeriodDays}
              </div>
              <div>
                <strong>Check Frequency Hours:</strong>{' '}
                {config.checkFrequencyHours}
              </div>
              <div>
                <strong>Auto Extension:</strong>{' '}
                {config.autoExtensionEnabled ? 'Enabled' : 'Disabled'}
              </div>
              <div>
                <strong>Membership Threshold:</strong>{' '}
                {config.significantChangeThresholds?.membershipPercent ?? 'N/A'}
                %
              </div>
              <div>
                <strong>Club Count Threshold:</strong>{' '}
                {config.significantChangeThresholds?.clubCountAbsolute ?? 'N/A'}
              </div>
            </div>
          </div>
        </div>
