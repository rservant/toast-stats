...
const AssessmentPanel: React.FC<Props> = ({
  districtId,
  selectedProgramYear,
  selectedDate,
}) => {
  const { data: cachedDatesData } = useDistrictCachedDates(districtId || '')
  const { isComputing, generateAssessment, fetchAssessment, deleteAssessment } =
    useAssessment()
  ...
  const datesInRange = React.useMemo(
    () =>
      allDates.filter(
        d =>
          d >= selectedProgramYear.startDate && d <= selectedProgramYear.endDate
      ),
  ...
  const latestDateInRange = datesInRange.length
    ? [...datesInRange].sort().pop()
    : undefined
  ...
  const previousMonthComplete =
    !!latestDateInRange && latestDateInRange >= prevLastStr

  // Helper: check if a given YYYY-MM month is complete according to cached dates
  const isMonthComplete = React.useCallback(
    (m: string) => {
  ...
  const defaultMonth = selectedDate
    ...
    : previousMonthComplete &&
        currentMonth >= selectedProgramYear.startDate &&
        currentMonth <= selectedProgramYear.endDate
      ? currentMonth
      : latestDateInRange
  ...
  const [persisted, setPersisted] = React.useState<PersistedAssessment | null>(
    null
  )
  ...
  const handleCompute = async () => {
    ...
    try {
      const res = await fetchAssessment(
        districtId,
        `${selectedProgramYear.startDate}/${selectedProgramYear.endDate}`,
        month
      )
      if (res && res.data) {
        console.log(
          'AssessmentPanel.handleCompute - fetched assessment:',
          res.data
        )
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const persistedRes = await fetchAssessment(
        districtId,
        `${selectedProgramYear.startDate}/${selectedProgramYear.endDate}`,
        month
      )
      if (persistedRes && persistedRes.data) {
        console.log(
          'AssessmentPanel.handleGenerate - fetched persisted:',
          persistedRes.data
        )
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      console.log(
        'AssessmentPanel.handleRegenerate - deleted existing assessment'
      )
      // Clear state
      setPersisted(null)
      setGenResult(null)
      ...
      if (persistedRes && persistedRes.data) {
        console.log(
          'AssessmentPanel.handleRegenerate - fetched persisted:',
          persistedRes.data
        )
        setPersisted(persistedRes.data)
      }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        <div>
          ...
          <div className="text-xs text-gray-500 mt-1">
            Months with cached data in the selected program year
          </div>
          {latestDateInRange && (
            <div className="text-xs text-gray-500 mt-1">
              Data available through {latestDateInRange}
            </div>
          )}
          {!canGenerateForSelectedMonth && month === currentMonth && (
            <div className="text-xs text-orange-600 mt-1">
              Previous month is not yet complete. Data must be available through{' '}
              {prevLastStr} to generate the current month's assessment.
            </div>
          )}
        </div>
        ...
        <div className="sm:col-span-2 flex items-end gap-2 flex-wrap">
          ...
          <button
            ...
            disabled={isGenerating || !month || !!persisted}
          >
            {isGenerating
              ? 'Generating...'
              : persisted
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div className="p-4 border rounded-md bg-gray-50">
            ...
            <div className="text-sm text-gray-900 mt-1">
              {persisted.created_at || persisted.timestamp || 'N/A'}
            </div>
          </div>
          <div className="p-4 border rounded-md bg-gray-50">
            <div className="text-xs font-semibold text-gray-600">
            ...
            <div
              className={`text-sm font-semibold mt-1 ${
                persisted.goal_1_status === 'On Track'
                  ? 'text-green-700'
                  : 'text-red-700'
              }`}
            >
          ...
          <div className="p-4 border rounded-md bg-gray-50">
            ...
            <div
              className={`text-sm font-semibold mt-1 ${
                persisted.goal_2_status === 'On Track'
                  ? 'text-green-700'
                  : 'text-red-700'
              }`}
            >
          ...
          <div className="p-4 border rounded-md bg-gray-50">
            ...
            <div
              className={`text-sm font-semibold mt-1 ${
                persisted.goal_3_status === 'On Track'
                  ? 'text-green-700'
                  : 'text-red-700'
              }`}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div className="p-4 border rounded-md">
            ...
            <div className="text-lg font-semibold">
              {summary.snapshotDate || 'N/A'}
            </div>
          </div>
          <div className="p-4 border rounded-md">
            <div className="text-xs text-gray-500">Total Clubs (snapshot)</div>
            <div className="text-lg font-semibold">
              {summary.totalClubs ?? 'N/A'}
            </div>
          </div>
          <div className="p-4 border rounded-md">
            <div className="text-xs text-gray-500">
              Total Membership (snapshot)
            </div>
            <div className="text-lg font-semibold">
              {summary.totalMembership ?? 'N/A'}
            </div>
          </div>
        </div>
      )}
