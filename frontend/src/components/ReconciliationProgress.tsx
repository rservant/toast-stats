...
export const ReconciliationProgress: React.FC<ReconciliationProgressProps> = ({
  ...
  const formatValue = (value: number, metricName: string) => {
    ...
    if (
      ...
      metricName.toLowerCase().includes('%')
    ) {
      return `${value.toFixed(1)}%`
    }
    if (
      metricName.toLowerCase().includes('count') ||
      metricName.toLowerCase().includes('total')
    ) {
  ...
  const significantChanges = metricChanges.filter(
      ...
      >
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">
            No metric changes detected for {targetMonth}
          </p>
        </div>
      </div>
    )
          ...
          <div className="space-y-3">
            {significantChanges.map((change, index) => (
              <div
                ...
                className={`rounded-lg border p-4 ${getChangeColor(change)}`}
              >
                ...
                {showDetails && (
                  <div className="mt-2 text-sm opacity-75">
                    <span>
                      {formatValue(change.previousValue, change.metricName)} →{' '}
                      {formatValue(change.currentValue, change.metricName)}
                    </span>
                  </div>
                )}
              </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {minorChanges.map((change, index) => (
              <div
                key={`minor-${index}`}
                className={`rounded-lg border p-3 ${getChangeColor(change)}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {getChangeIcon(change)}
