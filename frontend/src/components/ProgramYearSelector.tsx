...
import { ProgramYear, formatProgramYear, getProgramYearProgress } from '../utils/programYear';
...
export const ProgramYearSelector: React.FC<ProgramYearSelectorProps> = ({
  ...
  const progress = showProgress ? getProgramYearProgress(selectedProgramYear) : null;

  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      <label htmlFor="program-year-selector" className="text-xs sm:text-sm font-medium text-gray-700">
        Program Year
      </label>
      <div className="relative">
        ...
        >
          {availableProgramYears.map((programYear) => (
            <option key={programYear.year} value={programYear.year} className="text-gray-900 bg-white">
