...
describe('Fallback Metrics Tracking Accuracy - Property-Based Tests', () => {
      ...
      fc.asyncProperty(
        ...
        async navigationSequence => {
          ...
          for (const { date, scenario } of navigationSequence) {
            const wasCachedBefore = collector.hasCachedFallback(date)

            // Simulate navigation
            const result = collector.testSimulateNavigation(date, scenario)
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (date, repeatCount) => {
          ...
          const firstResult = collector.testSimulateNavigation(
            date,
            'fallback-success'
          )
          expect(firstResult.usedCachedFallback).toBe(false)

          let metrics = collector.getFallbackMetrics()
          ...
          // Final metrics check
          metrics = collector.getFallbackMetrics()
      ...
      fc.asyncProperty(
        ...
        async (datesToPreCache, datesToNavigate) => {
          ...
          for (const date of datesToPreCache) {
            collector.testCacheFallbackKnowledge(date, {
              fallbackMonth: 6,
              fallbackYear: 2024,
   ...
   *
   * A new ToastmastersCollector instance should have all metrics initialized to zero.
   */
      ...
      fc.asyncProperty(
        ...
        async dates => {
          ...
          for (const date of dates) {
            collector.testSimulateNavigation(date, 'standard-success')
          }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async dates => {
          ...
          for (const date of dates) {
            collector.testSimulateNavigation(date, 'fallback-failure')
          }
