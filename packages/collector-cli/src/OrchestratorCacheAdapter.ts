/**
 * Cache Adapter for Collector Orchestrator
 *
 * Implements ICollectorCache to provide file-system backed caching for the
 * collector orchestrator. Writes metadata in the full format expected by the
 ...
 * This ensures the backend can read files written by the collector-cli
 */
...
/**
 * Simple cache adapter that implements ICollectorCache for the orchestrator
 * Writes metadata in the full format expected by the backend
 */
export class OrchestratorCacheAdapter implements ICollectorCache {
  ...
  async getCacheMetadata(date: string): Promise<CacheMetadata | null> {
    ...
    try {
      ...
      // Convert to simple CacheMetadata for ICollectorCache interface
