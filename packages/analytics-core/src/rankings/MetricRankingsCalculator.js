...
export class MetricRankingsCalculator {
    ...
    calculateRegionRank(districtId, metric, allDistrictsRankings) {
        ...
        const targetDistrict = allDistrictsRankings.rankings.find(r => r.districtId === districtId);
        ...
        const districtsInRegion = allDistrictsRankings.rankings.filter(r => isValidRegion(r.region) && r.region === districtRegion);
    ...
    calculateMetricRankings(districtId, metric, allDistrictsRankings) {
        ...
        const worldPercentile = this.calculateWorldPercentile(worldRank, totalDistricts);
        // Requirements 3.1-3.5, 6.1: Calculate region rank
        const regionRankResult = this.calculateRegionRank(districtId, metric, allDistrictsRankings);
        return {
            worldRank,
            worldPercentile,
