 ...
 */
import { getDCPCheckpoint, getCurrentProgramMonth, getMonthName, } from './AnalyticsUtils.js';
/**
...
export class ClubHealthAnalyticsModule {
     ...
     */
    generateClubHealthData(snapshots) {
        ...
        return {
            ...
            interventionRequiredClubs: clubTrends.filter(c => c.currentStatus === 'intervention-required'),
        };
    }
    /**
     ...
     */
    identifyAtRiskClubs(districtId, snapshots) {
        if (snapshots.length === 0) {
     ...
     */
    getClubTrends(districtId, clubId, snapshots) {
        if (snapshots.length === 0) {
     ...
     */
    assessClubHealth(clubTrend, club, snapshotDate) {
        const riskFactors = [];
        // Get current membership from the latest trend data point
        const currentMembership = clubTrend.membershipTrend[clubTrend.membershipTrend.length - 1]?.count ??
            club.membershipCount;
        // Get current DCP goals from the latest trend data point
        const currentDcpGoals = clubTrend.dcpGoalsTrend[clubTrend.dcpGoalsTrend.length - 1]
            ?.goalsAchieved ?? club.dcpGoals;
        ...
        if (currentMembership < 12 && netGrowth < 3) {
            ...
            riskFactors.push(`Net growth since July: ${netGrowth} (need 3+ to override)`);
        }
        else {
            ...
            if (membershipRequirementMet && dcpCheckpointMet && cspRequirementMet) {
                ...
                if (!membershipRequirementMet) {
                    riskFactors.push(`Membership below threshold (${currentMembership} members, need 20+ or net growth 3+)`);
                }
                // Requirement 4.3: Add specific reason for DCP checkpoint not met
                if (!dcpCheckpointMet) {
                    const monthName = getMonthName(currentMonth);
                    riskFactors.push(`DCP checkpoint not met: ${currentDcpGoals} goal${currentDcpGoals !== 1 ? 's' : ''} achieved, ${requiredDcpCheckpoint} required for ${monthName}`);
                }
        ...
        clubTrend.healthScore = this.calculateClubHealthScore(currentMembership, currentDcpGoals);
    }
    /**
    ...
    countVulnerableClubs(snapshot) {
        ...
        return snapshot.clubs.filter(club => {
            ...
            const allRequirementsMet = membershipRequirementMet && dcpCheckpointMet && cspSubmitted;
     ...
     */
    countInterventionRequiredClubs(snapshot) {
        return snapshot.clubs.filter(club => {
            const membership = club.membershipCount;
            const netGrowth = this.calculateNetGrowth(club);
     ...
     */
    identifyDistinguishedLevel(clubTrend, club) {
        ...
        clubTrend.distinguishedLevel = this.determineDistinguishedLevel(currentDcpGoals, currentMembership, netGrowth);
    }
}
