...
import type { ClubTrend, ClubHealthStatus } from '../types.js'
import {
  parseIntSafe,
  parseIntOrUndefined,
  ensureString,
...
// ========== Backend-specific types (to be removed in task 4.2) ==========

/**
 * Backend-specific interface for data source access.
 * Will be removed in task 4.2 - module will accept DistrictStatistics[] directly.
 */
interface IAnalyticsDataSource {
  getSnapshotsInRange(
    startDate?: string,
    endDate?: string
  ): Promise<Array<{ snapshotId: string; dataAsOfDate: string }>>
  getLatestSnapshot(): Promise<{ snapshot_id: string } | null>
  getDistrictData(
    snapshotId: string,
    districtId: string
 ...
 */
interface DistrictStatisticsBackend {
  districtId: string
  asOfDate: string
...
/**
 * Backend-specific cache entry type.
 * Will be removed in task 4.2 - module will work with DistrictStatistics[] directly.
 */
...
/**
 * Scraped record type - represents a row from CSV data.
 */
type ScrapedRecord = Record<string, string | number | null | undefined>
...
export class ClubHealthAnalyticsModuleBackend {
   ...
   */
  countVulnerableClubs(entry: DistrictCacheEntry): number {
   ...
   */
  countInterventionRequiredClubs(entry: DistrictCacheEntry): number {
    return entry.clubPerformance.filter(club => {
      const membership = parseIntSafe(
        club['Active Members'] ||
          club['Active Membership'] ||
   ...
   * Uses new classification logic based on monthly DCP checkpoints
   */
  countThrivingClubs(entry: DistrictCacheEntry): number {
    return entry.clubPerformance.filter(club => {
      const membership = parseIntSafe(
        club['Active Members'] ||
          club['Active Membership'] ||
   ...
   */
  getCSPStatus(club: ScrapedRecord): boolean {
   ...
   */
  calculateNetGrowth(club: ScrapedRecord): number {
  ...
  private mapDistrictStatisticsToEntry(
    stats: DistrictStatisticsBackend,
    snapshotDate: string
  ): DistrictCacheEntry {
    return {
