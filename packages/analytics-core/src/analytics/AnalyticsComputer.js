...
import { findPreviousProgramYearDate, calculatePercentageChange, } from './AnalyticsUtils.js';
import { calculateGrowthTargets, calculatePercentageTargets, determineAchievedLevel, } from './TargetCalculator.js';
/**
...
export class AnalyticsComputer {
     ...
     */
    async computeDistrictAnalytics(districtId, snapshots, options) {
        ...
        const distinguishedClubs = this.distinguishedModule.generateDistinguishedClubCounts(sortedSnapshots);
        // distinguishedClubsList: detailed array for drill-down (Requirements 2.2)
        const distinguishedClubsList = this.distinguishedModule.generateDistinguishedClubSummaries(sortedSnapshots);
        // Get thriving count from latest snapshot for projection
        // Requirements: 1.1 (projected-year-end-simplification)
        const latestSnapshotForProjection = sortedSnapshots[sortedSnapshots.length - 1];
        const thrivingCount = latestSnapshotForProjection
            ? this.clubHealthModule.countThrivingClubs(latestSnapshotForProjection)
            : 0;
        const distinguishedProjection = this.distinguishedModule.generateDistinguishedProjection(sortedSnapshots, thrivingCount);
        ...
        const membershipChange = this.calculateMembershipChangeWithBase(sortedSnapshots, options?.allDistrictsRankings, districtId);
        ...
        const membershipAnalytics = this.computeMembershipAnalytics(districtId, sortedSnapshots);
        const vulnerableClubs = this.computeVulnerableClubs(districtId, clubHealth);
        const leadershipInsights = this.computeLeadershipInsights(districtId, sortedSnapshots);
        const distinguishedClubAnalytics = this.computeDistinguishedClubAnalytics(districtId, sortedSnapshots);
        const yearOverYear = this.computeYearOverYear(districtId, sortedSnapshots, currentDate);
        // Requirement 5.2 (per-metric-rankings): Pass allDistrictsRankings to computePerformanceTargets
        const performanceTargets = this.computePerformanceTargets(districtId, sortedSnapshots, options?.allDistrictsRankings);
     ...
     */
    calculateMembershipChangeWithBase(snapshots, allDistrictsRankings, districtId) {
        // Requirement 1.2: Log warning when rankings data is not provided
        if (!allDistrictsRankings) {
            logger.warn('All districts rankings data is undefined, cannot perform rankings-based membership change calculation', { districtId });
        }
        ...
        let districtRanking = allDistrictsRankings?.rankings.find(r => r.districtId === districtId);
        ...
        if (!districtRanking && allDistrictsRankings) {
            ...
            districtRanking = allDistrictsRankings.rankings.find(r => this.normalizeDistrictId(r.districtId) === normalizedSearchId);
            // Requirement 1.1: Log warning when district is not found in rankings after both lookups
            if (!districtRanking) {
                const availableDistrictIds = allDistrictsRankings.rankings.map(r => r.districtId);
                logger.warn('District not found in all districts rankings after exact and normalized lookup', {
        ...
        if (districtRanking && (paymentBase === undefined || paymentBase === null)) {
            logger.warn('Matched district ranking has null or undefined paymentBase, falling back to snapshot-based calculation', {
                districtId,
        ...
        if (paymentBase !== undefined && paymentBase !== null) {
            ...
            const currentPayments = districtRanking?.totalPayments ??
                (latestSnapshot
        ...
        const totalPayments = latestSnapshotFallback.clubs.reduce((sum, club) => sum + club.paymentsCount, 0);
        const totalMembershipBase = latestSnapshotFallback.clubs.reduce((sum, club) => sum + club.membershipBase, 0);
     ...
     */
    computeMembershipAnalytics(districtId, snapshots) {
        ...
        const membershipTrends = this.membershipModule.generateMembershipTrends(districtSnapshots);
        ...
        const membershipChange = this.membershipModule.calculateMembershipChange(districtSnapshots);
    ...
    calculateGrowthRate(snapshots) {
        ...
        const growthRate = ((currentMembership - initialMembership) / initialMembership) * 100;
     ...
     */
    calculateRetentionRate(snapshots) {
        if (snapshots.length === 0) {
            return 0;
        }
        const latestSnapshot = snapshots[snapshots.length - 1];
        ...
        const totalMembership = this.membershipModule.getTotalMembership(latestSnapshot);
     ...
     */
    computeVulnerableClubs(districtId, clubHealth) {
        return {
            districtId,
     ...
     */
    computeLeadershipInsights(districtId, snapshots) {
        // Filter snapshots for the requested district
        const districtSnapshots = snapshots.filter(s => s.districtId === districtId);
        // Calculate date range
        const dateRange = this.calculateDateRange(districtSnapshots);
        ...
        const insights = this.leadershipModule.generateLeadershipInsights(districtSnapshots);
        ...
        const trainingCompletionRate = this.calculateTrainingCompletionRate(insights);
        ...
        const topPerformingDivisions = insights.summary.topPerformingDivisions.map((div, index) => ({
            divisionId: div.divisionId,
            divisionName: div.divisionName,
            rank: index + 1,
            score: div.score,
            clubCount: this.getDivisionClubCount(div.divisionId, districtSnapshots),
            membershipTotal: this.getDivisionMembershipTotal(div.divisionId, districtSnapshots),
        }));
        // Identify areas needing support (low performing areas)
        const areasNeedingSupport = this.identifyAreasNeedingSupport(insights, districtSnapshots);
        return {
            districtId,
            dateRange,
     ...
     */
    calculateOfficerCompletionRate(insights) {
        if (insights.leadershipScores.length === 0) {
            return 0;
        }
        // Average health score across divisions represents officer effectiveness
        const totalHealthScore = insights.leadershipScores.reduce((sum, score) => sum + score.healthScore, 0);
     ...
     */
    calculateTrainingCompletionRate(insights) {
        if (insights.leadershipScores.length === 0) {
            return 0;
        }
        // Average DCP score across divisions represents training completion
        const totalDcpScore = insights.leadershipScores.reduce((sum, score) => sum + score.dcpScore, 0);
     ...
     */
    getDivisionClubCount(divisionId, snapshots) {
        const latestSnapshot = snapshots[snapshots.length - 1];
        if (!latestSnapshot) {
            return 0;
        }
     ...
     */
    getDivisionMembershipTotal(divisionId, snapshots) {
        const latestSnapshot = snapshots[snapshots.length - 1];
        if (!latestSnapshot) {
            return 0;
        }
     ...
     */
    identifyAreasNeedingSupport(insights, snapshots) {
        const latestSnapshot = snapshots[snapshots.length - 1];
        if (!latestSnapshot) {
            return [];
        }
        ...
        const lowPerformingAreas = insights.areaDirectorCorrelations
            .filter(area => area.activityIndicator === 'low' || area.correlation === 'negative')
            .map(area => {
            const areaClubs = latestSnapshot.clubs.filter(club => club.areaId === area.areaId);
            const membershipTotal = areaClubs.reduce((sum, club) => sum + club.membershipCount, 0);
            return {
                areaId: area.areaId,
     ...
     */
    computeDistinguishedClubAnalytics(districtId, snapshots) {
        // Filter snapshots for the requested district
        const districtSnapshots = snapshots.filter(s => s.districtId === districtId);
        // Calculate date range
        const dateRange = this.calculateDateRange(districtSnapshots);
        ...
        const distinguishedClubs = this.distinguishedModule.generateDistinguishedClubCounts(districtSnapshots);
        // Generate distinguished club summaries list
        const distinguishedClubsList = this.distinguishedModule.generateDistinguishedClubSummaries(districtSnapshots);
        // Generate distinguished projection
        // Get thriving count from latest snapshot for projection
        // Requirements: 1.1 (projected-year-end-simplification)
        const latestSnapshotForProjection = districtSnapshots[districtSnapshots.length - 1];
        const thrivingCount = latestSnapshotForProjection
            ? this.clubHealthModule.countThrivingClubs(latestSnapshotForProjection)
            : 0;
        const distinguishedProjection = this.distinguishedModule.generateDistinguishedProjection(districtSnapshots, thrivingCount);
        // Calculate progress by level with trends
        const progressByLevel = this.calculateProgressByLevel(distinguishedClubs, distinguishedProjection, districtSnapshots);
        return {
            districtId,
            dateRange,
            distinguishedClubs,
     ...
     */
    calculateProgressByLevel(counts, projection, snapshots) {
     ...
     */
    calculateDistinguishedTrends(snapshots) {
        if (snapshots.length < 2) {
            return {
                smedley: 'stable',
                presidents: 'stable',
        ...
        const firstCounts = this.distinguishedModule.generateDistinguishedClubCounts([firstSnapshot]);
        const lastCounts = this.distinguishedModule.generateDistinguishedClubCounts([lastSnapshot]);
        return {
            smedley: this.determineTrendFromCounts(firstCounts.smedley, lastCounts.smedley),
            presidents: this.determineTrendFromCounts(firstCounts.presidents, lastCounts.presidents),
            select: this.determineTrendFromCounts(firstCounts.select, lastCounts.select),
            distinguished: this.determineTrendFromCounts(firstCounts.distinguished, lastCounts.distinguished),
        };
    }
    /**
     ...
     */
    determineTrendFromCounts(first, last) {
        if (last > first) {
            return 'improving';
        }
        else if (last < first) {
     ...
     */
    computeYearOverYear(districtId, snapshots, currentDate) {
        // Filter snapshots for the requested district
        const districtSnapshots = snapshots.filter(s => s.districtId === districtId);
        ...
        const currentSnapshot = this.findSnapshotForDate(districtSnapshots, currentDate);
        if (!currentSnapshot) {
            return {
                districtId,
                currentDate,
        ...
        const previousSnapshot = this.findSnapshotForDate(districtSnapshots, previousYearDate);
        if (!previousSnapshot) {
            return {
                districtId,
                currentDate,
                previousYearDate,
                dataAvailable: false,
                message: 'Insufficient historical data for year-over-year comparison. Previous year data not available.',
            };
        }
        // Compute metrics for current and previous snapshots
        const metrics = this.computeYearOverYearMetrics(currentSnapshot, previousSnapshot);
        // Compute multi-year trends if sufficient data available
        const multiYearTrends = this.computeMultiYearTrends(districtSnapshots, currentDate);
        return {
            districtId,
            currentDate,
            previousYearDate,
     ...
     */
    findSnapshotForDate(snapshots, targetDate) {
        if (snapshots.length === 0) {
            return undefined;
        }
        ...
        for (const snapshot of snapshots) {
            const diff = Math.abs(new Date(snapshot.snapshotDate).getTime() -
     ...
     */
    computeYearOverYearMetrics(currentSnapshot, previousSnapshot) {
        // Membership comparison
        const currentMembership = this.membershipModule.getTotalMembership(currentSnapshot);
        const previousMembership = this.membershipModule.getTotalMembership(previousSnapshot);
        const membership = this.createMetricComparison(currentMembership, previousMembership);
        // Distinguished clubs comparison
        const currentDistinguished = this.distinguishedModule.generateDistinguishedClubCounts([
            currentSnapshot,
        ]);
        const previousDistinguished = this.distinguishedModule.generateDistinguishedClubCounts([
            previousSnapshot,
        ]);
        const distinguishedClubs = this.createMetricComparison(currentDistinguished.total, previousDistinguished.total);
        ...
        const clubHealth = {
            thrivingClubs: this.createMetricComparison(currentHealth.thrivingClubs.length, previousHealth.thrivingClubs.length),
            vulnerableClubs: this.createMetricComparison(currentHealth.vulnerableClubs.length, previousHealth.vulnerableClubs.length),
            interventionRequiredClubs: this.createMetricComparison(currentHealth.interventionRequiredClubs.length, previousHealth.interventionRequiredClubs.length),
        };
        // DCP goals comparison
        const currentTotalGoals = currentSnapshot.clubs.reduce((sum, club) => sum + club.dcpGoals, 0);
        const previousTotalGoals = previousSnapshot.clubs.reduce((sum, club) => sum + club.dcpGoals, 0);
        const currentAvgGoals = currentSnapshot.clubs.length > 0
            ? Math.round((currentTotalGoals / currentSnapshot.clubs.length) * 10) /
                10
            : 0;
        const previousAvgGoals = previousSnapshot.clubs.length > 0
            ? Math.round((previousTotalGoals / previousSnapshot.clubs.length) * 10) / 10
            : 0;
        const dcpGoals = {
            totalGoals: this.createMetricComparison(currentTotalGoals, previousTotalGoals),
            averagePerClub: this.createMetricComparison(currentAvgGoals, previousAvgGoals),
        };
        // Club count comparison
        const clubCount = this.createMetricComparison(currentSnapshot.clubs.length, previousSnapshot.clubs.length);
        return {
            membership,
            distinguishedClubs,
            clubHealth,
     ...
     */
    createMetricComparison(current, previous) {
     ...
     */
    computeMultiYearTrends(snapshots, currentDate) {
        if (snapshots.length < 2) {
            return undefined;
        }
        ...
        for (let yearOffset = 0; yearOffset < 5; yearOffset++) {
            ...
            const distinguishedCounts = this.distinguishedModule.generateDistinguishedClubCounts([snapshot]);
            const totalDcpGoals = snapshot.clubs.reduce((sum, club) => sum + club.dcpGoals, 0);
            trends.push({
                year: targetYear,
     ...
     */
    computePerformanceTargets(districtId, snapshots, allDistrictsRankings) {
        // Filter snapshots for the requested district
        const districtSnapshots = snapshots.filter(s => s.districtId === districtId);
        ...
        let paidClubsRankings = nullRankings;
        let membershipPaymentsRankings = nullRankings;
        let distinguishedClubsRankings = nullRankings;
        if (allDistrictsRankings) {
            // Requirement 1.2: Per-metric world rank for paid clubs SHALL equal clubsRank
            paidClubsRankings = this.metricRankingsCalculator.calculateMetricRankings(districtId, 'clubs', allDistrictsRankings);
            // Requirement 1.3: Per-metric world rank for membership payments SHALL equal paymentsRank
            membershipPaymentsRankings =
                this.metricRankingsCalculator.calculateMetricRankings(districtId, 'payments', allDistrictsRankings);
            // Requirement 1.4: Per-metric world rank for distinguished clubs SHALL equal distinguishedRank
            distinguishedClubsRankings =
                this.metricRankingsCalculator.calculateMetricRankings(districtId, 'distinguished', allDistrictsRankings);
        }
        ...
        const districtRanking = allDistrictsRankings?.rankings.find(r => r.districtId === districtId);
        ...
        const paidClubsTargets = paidClubBase !== null ? calculateGrowthTargets(paidClubBase) : null;
        const membershipPaymentsTargets = paymentBase !== null ? calculateGrowthTargets(paymentBase) : null;
        const distinguishedClubsTargets = paidClubBase !== null ? calculatePercentageTargets(paidClubBase) : null;
        // Handle empty snapshots case
        if (districtSnapshots.length === 0) {
            return {
                districtId,
        ...
        const currentMembership = this.membershipModule.getTotalMembership(latestSnapshot);
        ...
        const membershipTarget = Math.ceil(baseMembership * (1 + membershipGrowthRate));
        // Use AreaDivisionRecognitionModule to get area recognition data
        const areaRecognitions = this.areaDivisionRecognitionModule.calculateAreaRecognition(latestSnapshot);
        // Calculate total paid clubs and distinguished clubs from area recognition
        const totalPaidClubs = areaRecognitions.reduce((sum, area) => sum + area.paidClubs, 0);
        const currentDistinguished = areaRecognitions.reduce((sum, area) => sum + area.distinguishedClubs, 0);
        ...
        const distinguishedProgress = distinguishedTarget > 0 ? currentDistinguished / distinguishedTarget : 0;
        const clubGrowthProgress = clubGrowthTarget > 0 ? clubGrowth / clubGrowthTarget : 0;
        ...
        return {
            ...
            paidClubsAchievedLevel: determineAchievedLevel(totalPaidClubs, paidClubsTargets),
            membershipPaymentsAchievedLevel: determineAchievedLevel(currentMembershipPayments, membershipPaymentsTargets),
            distinguishedClubsAchievedLevel: determineAchievedLevel(currentDistinguished, distinguishedClubsTargets),
        };
    }
    /**
     ...
     */
    buildClubTrendsIndex(districtId, clubHealth) {
