 ...
 */
import { parseIntSafe, parseIntOrUndefined, ensureString, getDCPCheckpoint, getCurrentProgramMonth, getMonthName, } from './AnalyticsUtils.js';
/**
...
export class ClubHealthAnalyticsModuleBackend {
    ...
    async analyzeClubTrends(_districtId, dataEntries) {
        ...
        for (const record of latestEntry.districtPerformance) {
            ...
            const clubId = ensureString(record['Club'] || record['Club Number'] || record['Club ID']);
            if (clubId) {
        ...
        for (const club of latestEntry.clubPerformance) {
            ...
            const membershipPayments = this.extractMembershipPayments(districtRecord ?? club);
    ...
    calculateNetGrowth(club) {
        const currentMembers = parseIntSafe(club['Active Members'] || club['Active Membership'] || club['Membership']);
    ...
    extractMembershipPayments(club) {
        return {
            ...
            octoberRenewals: parseIntOrUndefined(club['Oct. Ren.'] ?? club['Oct. Ren']),
    ...
    identifyDistinguishedLevel(clubTrend, latestClubData) {
        ...
        const distinguishedLevel = this.determineDistinguishedLevel(currentDcpGoals, currentMembership, netGrowth);
