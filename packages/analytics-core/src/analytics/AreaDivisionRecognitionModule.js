...
export class AreaDivisionRecognitionModule {
     ...
     */
    calculateAreaRecognition(snapshot) {
     ...
     */
    calculateDivisionRecognition(snapshot) {
     ...
     */
    analyzeAreaRecognition(snapshot) {
        // Group clubs by area
        ...
        return Array.from(areaMap.values()).map(area => this.calculateSingleAreaRecognition(area.areaId, area.areaName, area.divisionId, area.clubs));
    }
    /**
     * Calculate recognition for a single area
     */
    calculateSingleAreaRecognition(areaId, areaName, divisionId, clubs) {
        const totalClubs = clubs.length;
        const paidClubs = clubs.filter(club => this.isClubPaid(club)).length;
        const distinguishedClubs = clubs.filter(club => this.isClubPaid(club) && this.isClubDistinguished(club)).length;
        ...
        const recognitionLevel = this.determineAreaRecognitionLevel(meetsPaidThreshold, distinguishedClubsPercent);
        const meetsDistinguishedThreshold = this.checkDistinguishedThreshold(recognitionLevel, distinguishedClubsPercent, DAP_DISTINGUISHED_THRESHOLD);
        return {
            areaId,
            areaName,
            divisionId,
     ...
     */
    determineAreaRecognitionLevel(meetsPaidThreshold, distinguishedPercent) {
     ...
     */
    analyzeDivisionRecognition(snapshot) {
        ...
        for (const area of areaRecognitions) {
            if (!divisionMap.has(area.divisionId)) {
                ...
                const divisionClub = snapshot.clubs.find(club => club.divisionId === area.divisionId);
        ...
        return Array.from(divisionMap.values()).map(division => this.calculateSingleDivisionRecognition(division.divisionId, division.divisionName, division.areas));
    }
    /**
     * Calculate recognition for a single division
     */
    calculateSingleDivisionRecognition(divisionId, divisionName, areas) {
        const totalAreas = areas.length;
        const paidAreas = areas.filter(area => this.isAreaPaid(area)).length;
        const distinguishedAreas = areas.filter(area => this.isAreaPaid(area) && area.recognitionLevel !== 'NotDistinguished').length;
        // Calculate percentages
        ...
        // Determine recognition level based on thresholds
        const recognitionLevel = this.determineDivisionRecognitionLevel(meetsPaidThreshold, distinguishedAreasPercent);
        const meetsDistinguishedThreshold = this.checkDistinguishedThreshold(recognitionLevel, distinguishedAreasPercent, DDP_DISTINGUISHED_THRESHOLD);
        return {
            divisionId,
            divisionName,
            totalAreas,
     ...
     */
    determineDivisionRecognitionLevel(meetsPaidThreshold, distinguishedPercent) {
        if (!meetsPaidThreshold) {
            return 'NotDistinguished';
        }
        // Check from highest to lowest (ordinal)
     ...
     */
    checkDistinguishedThreshold(recognitionLevel, distinguishedPercent, baseThreshold) {
