 ...
 * Will be removed in task 3.2 - module will accept DistrictStatistics[] directly.
 */
 ...
 * Will be replaced with analytics-core DistrictStatistics in task 3.2.
 */
interface DistrictStatisticsBackend {
  districtId: string
  asOfDate: string
  districtPerformance?: Record<string, string | number>[]
  divisionPerformance?: Record<string, string | number>[]
  clubPerformance?: Record<string, string | number>[]
}

/**
 * Backend-specific cache entry type.
 * Will be removed in task 3.2 - module will work with DistrictStatistics[] directly.
 */
...
/**
 * Internal club trend type for membership analysis.
 */
interface ClubTrendInternal {
  clubId: string
  clubName: string
  divisionId: string
  divisionName: string
...
export class MembershipAnalyticsModuleBackend {
  ...
  private async analyzeClubTrends(
    _districtId: string,
    dataEntries: DistrictCacheEntry[]
  ): Promise<ClubTrendInternal[]> {
    const latestEntry = dataEntries[dataEntries.length - 1]
    if (!latestEntry) {
      return []
    }

    const clubMap = new Map<string, ClubTrendInternal>()
