...
### 2. `frontend/src/hooks/useIntegratedData.ts` — Entire file is dead
...
### 3. `frontend/src/hooks/useDailyReports.ts` — Only consumer is dead

`useDailyReports` is only imported by `useIntegratedData.ts` (item 2 above). No component or test imports it directly. If `useIntegratedData.ts` is removed, this file becomes completely orphaned.

### 4. `frontend/src/hooks/useResponsiveDesign.ts` — Not used by any component
...
### 5. `backend/src/routes/districts/backfill.ts` — Deprecated endpoint file
...
### 6. `PerDistrictFileSnapshotStore` alias in `SnapshotStore.ts`
...
### 7. `AreaProgressSummary` / `AreaProgressSummaryProps` / `AreaWithDivision` in `DivisionAreaProgressSummary.tsx`
...
### 8. `AreaRecognitionPanel` / `AreaRecognitionPanelProps` in `DivisionAreaRecognitionPanel.tsx`
...
### 9. `createSnapshotStore()` on `ProductionServiceFactory`
...
### 10. `AvailableProgramYearsResult` type alias
...
### 11. `backend/src/routes/admin.ts` — Deprecated re-export shim
...
The following high-priority items were identified and removed on 2026-02-16:
