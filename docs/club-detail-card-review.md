...
4. Those files are generated by `collector-cli` → `analytics-core` → `ClubHealthAnalyticsModule.analyzeClubTrends()` → written to `district_{id}_club-trends-index.json`
...
Toastmasters Dashboard CSV
  → collector-cli (Playwright scrape + CSV parse)
...
The `ClubTrend` object carries `octoberRenewals`, `aprilRenewals`, and `newMembers` fields (populated from the collector pipeline), but the modal doesn't render them anywhere. This is data that's computed, stored, served, and transferred to the frontend — then silently dropped.
